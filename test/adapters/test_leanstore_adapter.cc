...
TEST_F(TestLeanStoreAdapter, BasicTest) {
  auto adapter =
      std::make_unique<LeanStoreAdapter<benchmark::RelationTest>>(*db_);
  ...
  db_->worker_pool.ScheduleSyncJob(1, [&]() {
    db_->StartTransaction();
    adapter->Scan(
        benchmark::RelationTest::Key{0},
        [](const benchmark::RelationTest::Key &r_key,
           const benchmark::RelationTest &record) -> bool {
...
TEST_F(TestLeanStoreAdapter, VariableSizeTest) {
  auto adapter =
      std::make_unique<LeanStoreAdapter<benchmark::VariableSizeRelation>>(*db_);
  ...
  db_->worker_pool.ScheduleSyncJob(1, [&]() {
    ...
    adapter->Scan(
        ...
        [](const benchmark::VariableSizeRelation::Key &r_key,
           const benchmark::VariableSizeRelation &record) -> bool {
          auto key =
              static_cast<const benchmark::VariableSizeRelation::Key *>(&r_key);
          auto r =
              static_cast<const benchmark::VariableSizeRelation *>(&record);
          EXPECT_EQ(r->PayloadSize(), key->primary_id + sizeof(Integer));
          for (auto i = 0; i < r->size; i++) {
            EXPECT_EQ(key->primary_id, r->data[i]);
          }
          return key->primary_id < NO_RECORDS / 2;
        });
    for (int idx = 0; idx < NO_RECORDS; idx++) {
