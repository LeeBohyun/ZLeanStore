...
#define REQUEST_TIER(idx)                                                      \
  ({                                                                           \
    free_pages_->RequestFreeExtent(storage::ExtentList::ExtentSize(idx),       \
                                   accept_all_lbd_, start_pid, to_free_list);  \
  })
...
protected:
  ...
  static constexpr u64 EVICT_SIZE =
      64; // should be <= FLAGS_bm_aio_qd defined in main.cc
  static constexpr u32 WRITE_BUFFER_PARTITION_COUNT = 8;
  static constexpr u64 FLASH_BLOCK_SIZE = 8;
  ...
  std::unique_ptr<storage::space::SpaceManager> space_;

  // Misc shared properties
  std::function<bool(pageid_t)> accept_all_lbd_ =
      []([[maybe_unused]] pageid_t pid) { return true; };
  ...
  void SetupTestFile(bool setup_fd = false) {
    ...
    space_ = std::make_unique<storage::space::SpaceManager>();
    buffer_ = std::make_unique<buffer::BufferManager>(
        N_PAGES, PHYSICAL_CAP, EXTRA_NO_PG, EVICT_SIZE, is_running_,
        free_pages_.get(), space_.get(), is_running_);
    //  buffer_     = std::make_unique<buffer::BufferManager>(is_running_,
    //  free_pages_.get(), space_.get());
    log_ = std::make_unique<recovery::LogManager>(is_running_);
    txn_man_ = std::make_unique<transaction::TransactionManager>(buffer_.get(),
                                                                 log_.get());
  ...
  void ModifyPageContent(storage::Page *page) {
    std::memset(reinterpret_cast<u8 *>(page) + sizeof(storage::PageHeader), 7,
  ...
  void InitRandTransaction(wid_t w_id = 0) {
    LOG_INFO("init rand transaction");
    worker_thread_id = w_id;
    txn_man_->StartTransaction(transaction::Transaction::Type::USER,
                               transaction::IsolationLevel::READ_UNCOMMITTED,
