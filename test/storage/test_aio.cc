...
TEST_F(TestAioInterface, BasicTest) {
  auto aio_inf = storage::space::LibaioInterface(test_file_fd_, mem_);

  // Write Page 1 and Page 3
  auto dirty_pages = std::vector<pageid_t>{1, 3};
  for (auto pid : dirty_pages) {
    ModifyPageContent(&mem_[pid]);
    aio_inf.WritePage(pid);
  }

  // Read from the test file using mmap
  auto data = reinterpret_cast<u8 *>(
      mmap(nullptr, 4 * PAGE_SIZE, PROT_READ, MAP_PRIVATE, test_file_fd_, 0));
