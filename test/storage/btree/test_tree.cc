...
protected:
  ...
  void PrepareData(std::vector<std::pair<int, value_t>> &data,
                   bool get_permutation = false, bool prefill_tree = true) {
    for (size_t idx = 0; idx < NO_RECORDS; idx++) {
      data.emplace_back(static_cast<int>(__builtin_bswap32(idx + 1)),
                        static_cast<value_t>(idx * 100));
    }
    if (get_permutation) {
    ...
    if (prefill_tree) {
      for (auto &pair : data) {
        ...
        std::span payload{reinterpret_cast<u8 *>(&pair.second),
                          sizeof(value_t)};
  ...
  for (auto &pair : data) {
    ...
    auto found = tree_->LookUp(key, [&payload](std::span<u8> data) {
      ...
      for (size_t idx = 0; idx < data.size(); idx++) {
        EXPECT_EQ(payload[idx], data[idx]);
      }
    });
    ASSERT_TRUE(found);
  }
...
TEST_F(TestBTree, UpdateAndQuery) {
  ...
  for (auto idx = 0; idx < static_cast<int>(NO_RECORDS); idx++) {
    ...
    auto found = tree_->LookUp(key, [&payload](std::span<u8> found_payload) {
      ...
      for (size_t idx = 0; idx < found_payload.size(); idx++) {
        EXPECT_EQ(payload[idx], found_payload[idx]);
      }
    });
    ASSERT_TRUE(found);
  }
...
TEST_F(TestBTree, TreeScan) {
  ...
  auto read_cb = [&scan_result](std::span<u8> key,
                                std::span<u8> payload) -> bool {
    scan_result[LoadUnaligned<int>(key.data())] =
        LoadUnaligned<int>(payload.data());
    return true;
  };
...
TEST_F(TestBTree, ConcurrentInsertAndSearch) {
  ...
  for (u32 t_id = 0; t_id < NO_THREADS; t_id++) {
    threads[t_id] = std::thread([&, t_id]() {
      ...
      // it would take like a year to complete the tests for NO_RECORDS
      // operations
      ...
      for (size_t idx = 0; idx < NO_RECORDS / 10; idx++) {
        ...
        if (r == 0 && insert_idx < NO_RECORDS) {
          ...
          if (index_pos < data.size()) {
            std::span key{reinterpret_cast<u8 *>(&data[index_pos].first),
                          sizeof(int)};
            std::span payload{reinterpret_cast<u8 *>(&data[index_pos].second),
                              sizeof(int)};
            tree_->Insert(key, payload);
          ...
          while (!inserted_f[search_pos].load()) {
            search_pos = rand() % insert_idx.load();
          }
          std::span key{reinterpret_cast<u8 *>(&data[search_pos].first),
                        sizeof(int)};

          int found_data;
          auto found = tree_->LookUp(key, [&found_data](std::span<u8> data) {
            std::memcpy(&found_data, data.data(), data.size());
          });
