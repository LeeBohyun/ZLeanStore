...
template <class T> struct MockGuardO : public GuardO<T> {
  MockGuardO(buffer::BufferManager *buffer, pageid_t pid)
      : GuardO<T>(buffer, pid) {}
...
template <class T> struct MockGuardX : public GuardX<T> {
  MockGuardX(buffer::BufferManager *buffer, pageid_t pid)
      : GuardX<T>(buffer, pid) {}

  MOCK_METHOD0(Die, void());

  static void Dumb([[maybe_unused]] MockGuardX<T> &&parent,
                   [[maybe_unused]] MockGuardX<BTreeNode> &&child) {
