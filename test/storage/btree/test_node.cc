...
protected:
  ...
  void PrepareData(storage::BTreeNode *node,
    ...
    if (get_permutation) {
      for (size_t idx = 0; idx < rand() % NO_RECORDS; idx++) {
  ...
  template <typename value_t>
  auto
  PrepareSmallTree(storage::BTreeNode *parent, storage::BTreeNode *left,
                   storage::BTreeNode *right,
                   std::vector<std::pair<int, value_t>> &data,
    ...
    parent->right_most_child = pid_map[left];
    parent->count = 0; // right_most_child doesn't count as a kv in inner nodes
    ...
    left->SplitNode(parent, right, pid_map[left], pid_map[right], sep_info.slot,
                    {separator_key_, sep_info.len}, cmp_);
    return sep_info;
  }
};

TEST_F(TestBTreeNode, Constructor) {
  auto node = new (&mem_) storage::BTreeNode(true, 0);
  EXPECT_EQ(node->Ptr(), reinterpret_cast<u8 *>(&mem_));
  EXPECT_EQ(node->count, 0);
  EXPECT_EQ(node->space_used, 0);
  EXPECT_EQ(node->data_offset, PAGE_SIZE);
  EXPECT_EQ(node->prefix_len, 0);
  EXPECT_TRUE(mem_.dirty);
  EXPECT_TRUE(node->dirty);
  EXPECT_FALSE(node->IsInner());
  EXPECT_TRUE(node->IsLowerFenceInfinity());
  EXPECT_TRUE(node->IsUpperFenceInfinity());
}
...
TEST_F(TestBTreeNode, NormalOperation) {
  ...
  EXPECT_EQ(node->prefix_len, 0);
  EXPECT_EQ(node->count, NO_RECORDS);
  EXPECT_EQ(node->space_used, sizeof(int) * 2 * NO_RECORDS);
  EXPECT_EQ(node->data_offset, PAGE_SIZE - sizeof(int) * 2 * NO_RECORDS);
  ...
  for (auto &pair : data) {
    ...
    EXPECT_EQ(LoadUnaligned<int>(node->GetPayload(slot_id).data()),
              pair.second);
  }

  // Try clone and check hints
  auto clone = std::make_unique<storage::BTreeNode>(true, 0);
  storage::BTreeNode::CopyNodeContent(clone.get(), node);
  for (size_t idx = 0; idx < storage::BTreeNode::HINT_COUNT; idx++) {
    LOG_DEBUG("Clone's Hint[%ld]: %s - Original Hint[%ld]: %s", idx,
              fmt::format("{}", clone->hints[idx]).c_str(), idx,
              fmt::format("{}", node->hints[idx]).c_str());
    EXPECT_EQ(std::memcmp(clone->hints[idx].data(), node->hints[idx].data(), 4),
              0);
  }
...
TEST_F(TestBTreeNode, NormalOperationWithPrefix) {
  ...
  auto node = new (&mem_) storage::BTreeNode(true, 0);
  node->SetFences(std::span{lower_fence.data(), lower_fence.size()},
                  std::span{upper_fence.data(), upper_fence.size()}, cmp_);
  for (u8 idx = 0; idx < NO_RECORDS; idx++) {
    data.emplace_back(prefix + u8string{idx, idx, idx, idx},
                      static_cast<leng_t>(idx * 100));
  }
  for (size_t idx = 0; idx < rand() % NO_RECORDS; idx++) {
    std::next_permutation(data.begin(), data.end());
  }

  EXPECT_EQ(node->prefix_len, 6);
  auto expected_space_used = lower_fence.size() + upper_fence.size();
  EXPECT_EQ(node->space_used, expected_space_used);
  ...
  for (auto &pair : data) {
    ...
    node->InsertKeyValue(key, payload, cmp_);
    expected_space_used += sizeof(leng_t) + key.size() - node->prefix_len;
    EXPECT_EQ(node->space_used, expected_space_used);
  }
  EXPECT_EQ(node->count, NO_RECORDS);
  EXPECT_EQ(node->data_offset, PAGE_SIZE - expected_space_used);
  ...
  for (auto &pair : data) {
    ...
    // Search Hint test
    ASSERT_GT(node->prefix_len, 0);
    auto key_head = storage::BTreeNode::GetHead(
        pair.first.data() + node->prefix_len, key.size() - node->prefix_len);
    leng_t lower = 0;
    leng_t upper = node->count;
    node->SearchHint(key_head, lower, upper);
    ...
    LOG_DEBUG("Lower %u - upper %u - slot_id %d - key %s", lower, upper,
              slot_id, node->KeyToString(slot_id).c_str());
    EXPECT_TRUE(exact_found);
    ASSERT_TRUE((lower <= slot_id) && (slot_id <= upper));
    u8string queried_key(key.size(), 0);
    std::memcpy(queried_key.data(), node->GetPrefix(), node->prefix_len);
    std::memcpy(queried_key.data() + node->prefix_len, node->GetKey(slot_id),
                key.size() - node->prefix_len);
    EXPECT_EQ(queried_key, pair.first);
    EXPECT_EQ(LoadUnaligned<leng_t>(node->GetPayload(slot_id).data()),
              pair.second);
  }
...
TEST_F(TestBTreeNode, TestCopyRange) {
  auto node = new (&mem_) storage::BTreeNode(true, 0);
  auto other = std::make_unique<storage::BTreeNode>(true, 0);
  ...
  // Clone ops
  node->CopyKeyValueRange(other.get(), 0, 0, node->count);
  EXPECT_EQ(node->count, other->count);
  LOG_DEBUG("node count %u - other count %u", node->count, other->count);
  for (size_t idx = 0; idx < node->count; idx++) {
    ...
    EXPECT_EQ(node->slots[idx].payload_length,
              other->slots[idx].payload_length);
    ...
    for (size_t jdx = 0; jdx < node_payload.size(); jdx++) {
      EXPECT_EQ(node_payload[jdx], other_payload[jdx]);
    }
  }
}
...
TEST_F(TestBTreeNode, TestSplit) {
  auto node = new (&mem_) storage::BTreeNode(true, 0);
  auto new_leaf = std::make_unique<storage::BTreeNode>(true, 0);
  auto parent = std::make_unique<storage::BTreeNode>(false, 0);
  std::vector<std::pair<int, int>> data;
  std::unordered_map<storage::BTreeNode *, pageid_t> pid_map;

  // Prepare initial data
  auto sep_info =
      PrepareSmallTree<int>(parent.get(), node, new_leaf.get(), data, pid_map);
  EXPECT_EQ(parent->right_most_child, pid_map[new_leaf.get()]);
  ...
  EXPECT_LE(node->count, new_leaf->count);
  LOG_DEBUG("Node count %u, new leaf count %u", node->count, new_leaf->count);
  EXPECT_LE(sep_info.len,
            node->prefix_len + node->slots[node->count - 1].key_length);

  u8 last_key[node->prefix_len + node->slots[node->count - 1].key_length];
  std::memcpy(&last_key, new_leaf->GetPrefix(), new_leaf->prefix_len);
  std::memcpy(&last_key + new_leaf->prefix_len, new_leaf->GetKey(0),
              new_leaf->slots[0].key_length);
  for (size_t idx = 0; idx < sep_info.len; idx++) {
    EXPECT_EQ(separator_key_[idx], last_key[idx]);
  }
}

TEST_F(TestBTreeNode, TestSplitInnerNodes) {
  auto left = new (&mem_) storage::BTreeNode(false, 0);
  auto right = std::make_unique<storage::BTreeNode>(false, 0);
  auto parent = std::make_unique<storage::BTreeNode>(false, 0);
  ...
  // Prepare initial data
  auto sep_info = PrepareSmallTree<pageid_t>(parent.get(), left, right.get(),
                                             data, pid_map);
  EXPECT_EQ(parent->count, 1);
  EXPECT_EQ(parent->right_most_child, pid_map[right.get()]);
  EXPECT_EQ(parent->GetChild(0), pid_map[left]);
  EXPECT_EQ(left->right_most_child, NO_RECORDS / 2);

  LOG_DEBUG("Parent: %s", parent->ToString().c_str());
  LOG_DEBUG("Left: %s", left->ToString().c_str());
  LOG_DEBUG("Right: %s", right->ToString().c_str());
  EXPECT_LE(sep_info.len,
            left->prefix_len + left->slots[left->count - 1].key_length);

  u8 parent_key[parent->prefix_len +
                parent->slots[parent->count - 1].key_length];
  std::memcpy(&parent_key, parent->GetPrefix(), parent->prefix_len);
  std::memcpy(&parent_key + parent->prefix_len, parent->GetKey(0),
              parent->slots[0].key_length);
  for (size_t idx = 0; idx < sep_info.len; idx++) {
    EXPECT_EQ(separator_key_[idx], parent_key[idx]);
  }
}

TEST_F(TestBTreeNode, TestMergeLeafNodes) {
  auto left = new (&mem_) storage::BTreeNode(true, 0);
  auto right = std::make_unique<storage::BTreeNode>(true, 0);
  auto parent = std::make_unique<storage::BTreeNode>(false, 0);
  std::vector<std::pair<int, int>> data;
  std::unordered_map<storage::BTreeNode *, pageid_t> pid_map;
  ...
  EXPECT_EQ(parent->right_most_child, pid_map[right.get()]);
  EXPECT_EQ(right->count, NO_RECORDS);
  ...
  for (auto &pair : data) {
    ...
    // Search Hint test
    ASSERT_EQ(left->prefix_len, 0);
    auto key_head = storage::BTreeNode::GetHead(key.data(), sizeof(int));
    leng_t lower = 0;
    leng_t upper = right->count;
    ...
    EXPECT_EQ(LoadUnaligned<int>(right->GetPayload(slot_id).data()),
              pair.second);
  }
}
...
TEST_F(TestBTreeNode, TestMergeInnerNodes) {
  ...
  for (auto &pair : data) {
    ...
    EXPECT_TRUE((lower <= slot_id) && (slot_id <= upper));
    EXPECT_EQ(LoadUnaligned<int>(right->GetKey(slot_id)), pair.first);
    EXPECT_EQ(LoadUnaligned<pageid_t>(right->GetPayload(slot_id).data()),
              pair.second);
  }
}
