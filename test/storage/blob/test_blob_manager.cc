...
protected:
  static constexpr u64 BLOB_SIZE =
      18432; // 4.5 * PAGE_SIZE,  3 Extents < Blob < 4 Extents
  u8 *random_blob_[2];
...
TEST_P(TestBlobManager, InsertNewBlob) {
  ...
  auto stored_blob_ptr =
      reinterpret_cast<u8 *>(buffer_->ToPtr(blob_h->BlobID()));
  if (blob_likely_grow) {
  ...
  EXPECT_EQ(blob_h->PageCount(),
            std::ceil(static_cast<float>(BLOB_SIZE) / PAGE_SIZE));
  EXPECT_EQ(std::memcmp(stored_blob_ptr, blob_payload.data(), BLOB_SIZE), 0);
  for (auto idx = 0; idx < blob_h->extents.NumberOfExtents(); idx++) {
    EXPECT_EQ(blob_h->extents.extent_pid[idx],
              txn.ToFlushedLargePages()[idx].start_pid);
  }

  // All states should be UNLOCKED - check `prevent_evict` description
  for (auto &extent : blob_h->extents) {
    CHECK_EXTENT_PAGE_STATE(sync::PageState::UNLOCKED, extent.start_pid,
                            extent.page_cnt);
  }
  ...
  auto expected_phys_cnt = buffer_->physical_used_cnt_.load();
  {
    ...
    auto ret =
        pwrite(test_file_fd_, buffer_->ToPtr(blob_h->BlobID()),
               blob_h->PageCount() * PAGE_SIZE, blob_h->BlobID() * PAGE_SIZE);
  ...
  if (blob_likely_grow) {
    ASSERT_EQ(blob_h->extents.NumberOfExtents(),
              3); // 4.5 pages -> corresponds to three extents [1, 2, 4]
  } else {
    ASSERT_EQ(blob_h->extents.NumberOfExtents(),
              2); // 2 extents and the special block is used
    buffer_->EvictExtent(txn.ToEvictedExtents().back().start_pid,
                         txn.ToEvictedExtents().back().page_cnt);
  }
  EXPECT_EQ(txn.ToEvictedExtents().size(), 3);
  ASSERT_EQ(blob_h->extents.NumberOfExtents(),
            3 - static_cast<int>(!blob_likely_grow));
  if (FLAGS_blob_logging_variant == 0) {
    ASSERT_EXIT(stored_blob_ptr[0] = 0, ::testing::ExitedWithCode(1), ".*");
    expected_phys_cnt -= storage::ExtentList::TotalSizeExtents(
        blob_h->extents.NumberOfExtents() - 1);
    if (!blob_likely_grow) {
      expected_phys_cnt -= blob_h->extents.special_blk.page_cnt;
    }
  } else {
  ...
  case 0:
    expected_state = sync::PageStateMode::EVICTED;
    break;
  case 1:
    expected_state = sync::PageStateMode::UNLOCKED;
    break;
  case 2:
    expected_state = sync::PageStateMode::MARKED;
    break;
  default:
    UnreachableCode();
    break;
  ...
  blob_manager_->LoadBlob(
      BlobManager::active_blob, PAGE_SIZE, [&](std::span<const u8> blob) {
  ...
  CHECK_EXTENT_PAGE_STATE(expected_state, second_extent.start_pid,
                          second_extent.page_cnt);

  blob_manager_->LoadBlob(
      BlobManager::active_blob, BlobManager::active_blob->blob_size,
  ...
  EXPECT_THAT(blob_h->sha2_val,
              ::testing::ElementsAre(240, 55, 63, 44, 138, 135, 12, 194, 146,
                                     233, 55, 136, 33, 194, 149, 128, 34, 237,
                                     144, 228, 21, 199, 111, 124, 237, 167, 242,
                                     34, 246, 249, 33, 250));
  blob_manager_->UnloadAllBlobs();
  for (auto &extent : blob_h->extents) {
    CHECK_EXTENT_PAGE_STATE(sync::PageStateMode::UNLOCKED, extent.start_pid,
                            extent.page_cnt);
  }

  txn.ToFreeExtents().clear();
  // Now remove the Blob - there should be a single free range of 7 pages, start
  // at `extents[0].start_pid`
  ...
  if (blob_likely_grow) {
    ...
    EXPECT_THAT(temp,
                ::testing::ElementsAre(txn.ToEvictedExtents()[1].start_pid,
                                       txn.ToEvictedExtents()[2].start_pid));
  }
...
TEST_P(TestBlobManager, GrowExistingBlob) {
  ...
  auto expected_phys_cnt = buffer_->physical_used_cnt_.load();
  {
    // Mock: write BLOB to disk
    auto ret =
        pwrite(test_file_fd_, buffer_->ToPtr(blob->BlobID()),
               blob->PageCount() * PAGE_SIZE, blob->BlobID() * PAGE_SIZE);
  ...
  if (FLAGS_blob_logging_variant == 0) {
    expected_phys_cnt -= storage::ExtentList::TotalSizeExtents(
        blob->extents.NumberOfExtents() - 1);
    if (!blob_likely_grow) {
      expected_phys_cnt -= blob->extents.special_blk.page_cnt;
    }
  }
  EXPECT_EQ(buffer_->physical_used_cnt_.load(), expected_phys_cnt);
  ...
  auto grow_blob = blob_manager_->AllocateBlob(
      std::span<u8>{random_blob_[1], BLOB_SIZE}, blob);

  if (blob_likely_grow) {
    // Start txn log entry + PageImgEntry log entry
    auto exp_cursor = sizeof(recovery::LogEntry) +
                      sizeof(recovery::PageImgEntry) + PAGE_SIZE / 2;
    EXPECT_EQ(transaction::TransactionManager::active_txn.LogWorker().w_cursor,
              exp_cursor);
    // Evaluate Log value
    auto log_offset =
        sizeof(recovery::LogEntry) + sizeof(recovery::PageImgEntry);
    auto log_entry = &transaction::TransactionManager::active_txn.LogWorker()
                          .wal_buffer[log_offset];
    EXPECT_EQ(std::memcmp(log_entry, random_blob_[0], PAGE_SIZE / 2),
              0); // full of random_blob_[0][1]
  } else {
    // A whole new extent is allocated to store the content of the special block
    // Therefore, no log should be appended, i.e. only Start txn log entry if
    // the log buffer
    EXPECT_EQ(transaction::TransactionManager::active_txn.LogWorker().w_cursor,
              sizeof(recovery::LogEntry));
  }

  // Appending existing blob requires loading all of its extents into the buffer
  // manager The last extent should be marked as Dirty and is waiting to be
  // evicted & flushed to disk
  EXPECT_FALSE(BlobManager::extent_loaded.isEmpty());
  ...
  for (auto idx = 0UL; idx < BlobManager::extent_loaded.cardinality(); idx++) {
    EXPECT_TRUE(
        BlobManager::extent_loaded.contains(blob->extents.extent_pid[idx]));
  }

  // The last extent of `blob` has 3 remaining empty pages, which is not enough
  // to store BLOB_SIZE (requires 4 pages) Therefore, the expected flushed
  // extents should be the prev last one + 1 new extent, which == 2
  EXPECT_EQ(txn.ToFlushedLargePages().size(), 2);
  if (blob_likely_grow) {
    EXPECT_EQ(txn.ToFlushedLargePages()[0].start_pid,
              txn.ToEvictedExtents()[0].start_pid + 1);
    EXPECT_EQ(txn.ToFlushedLargePages()[0].page_cnt,
              txn.ToEvictedExtents()[0].page_cnt - 1);
  } else {
    EXPECT_EQ(txn.ToFlushedLargePages()[0].start_pid,
              txn.ToEvictedExtents()[0].start_pid);
    EXPECT_EQ(txn.ToFlushedLargePages()[0].page_cnt,
              txn.ToEvictedExtents()[0].page_cnt);
  }
  ...
  // First two extents are in SHARED (1 owner), last two extents are UNLOCKED +
  // prevent_evict==true
  ...
  if (FLAGS_blob_logging_variant == 0) {
    // Prev content is evicted & loaded again. The special block should also be
    // loaded if applicable
    expected_phys_cnt += storage::ExtentList::TotalSizeExtents(
        grow_blob->extents.NumberOfExtents() - 1);
    if (!blob_likely_grow) {
      expected_phys_cnt += blob->extents.special_blk.page_cnt;
    }
  } else {
    // Prev content is still in memory, no need to include it in the calculation
    expected_phys_cnt += storage::ExtentList::ExtentSize(
        grow_blob->extents.NumberOfExtents() - 1);
    // If there was a special block, then a new extent at
    //  index `grow_blob->extents.NumberOfExtents() - 2` will be allocated to
    //  store that block
    if (!blob_likely_grow) {
      expected_phys_cnt += storage::ExtentList::ExtentSize(
          grow_blob->extents.NumberOfExtents() - 2);
    }
  }
  EXPECT_EQ(buffer_->physical_used_cnt_.load(), expected_phys_cnt);
  ...
  {
    auto guard = storage::blob::PageAliasGuard(buffer_.get(), *grow_blob,
                                               grow_blob->blob_size);
    EXPECT_EQ(std::memcmp(guard.GetPtr(), random_blob_[0], BLOB_SIZE), 0);
    EXPECT_EQ(
        std::memcmp(guard.GetPtr() + BLOB_SIZE, random_blob_[1], BLOB_SIZE), 0);
  }
  EXPECT_THAT(grow_blob->sha2_val,
              ::testing::ElementsAre(131, 150, 200, 204, 16, 190, 179, 154, 149,
                                     104, 200, 138, 122, 19, 62, 59, 80, 140,
                                     136, 103, 91, 233, 104, 20, 82, 232, 60,
                                     96, 48, 203, 45, 160));

  // -------------------------------------------------------------------------------------
  {
    // Mock: write the grow BLOB to disk, no need to do it correctly according
    // to dirty state
    for (auto &lp : txn.ToFlushedLargePages()) {
      auto ret = pwrite(test_file_fd_, buffer_->ToPtr(lp.start_pid),
                        lp.page_cnt * PAGE_SIZE, lp.start_pid * PAGE_SIZE);
  ...
  for (idx =
           grow_blob->extents.NumberOfExtents() - txn.ToEvictedExtents().size();
  ...
  for (idx = 0; idx < grow_blob->extents.NumberOfExtents() -
                          txn.ToEvictedExtents().size();
       idx++) {
    auto extent = grow_blob->extents[idx];
    CHECK_EXTENT_PAGE_STATE(1, extent.start_pid, extent.page_cnt);
  }
  ...
  blob_manager_->LoadBlob(
      BlobManager::active_blob, PAGE_SIZE,
      [&](std::span<const u8> blob_payload) {
        EXPECT_EQ(blob_payload.size(), PAGE_SIZE);
        EXPECT_EQ(std::memcmp(blob_payload.data(), random_blob_[0], PAGE_SIZE),
                  0);
      });
  EXPECT_FALSE(BlobManager::extent_loaded.isEmpty());
  EXPECT_EQ(BlobManager::extent_loaded.cardinality(), 2);
  ...
  blob_manager_->LoadBlob(
      ...
      [&](std::span<const u8> blob_payload) {
        ...
        EXPECT_THAT(dump.sha2_val,
                    ::testing::ElementsAre(
                        131, 150, 200, 204, 16, 190, 179, 154, 149, 104, 200,
                        138, 122, 19, 62, 59, 80, 140, 136, 103, 91, 233, 104,
                        20, 82, 232, 60, 96, 48, 203, 45, 160));
        EXPECT_EQ(std::memcmp(blob_payload.data(), random_blob_[0], BLOB_SIZE),
                  0);
        EXPECT_EQ(std::memcmp(blob_payload.data() + BLOB_SIZE, random_blob_[1],
                              BLOB_SIZE),
                  0);
      });
  ...
  EXPECT_EQ(free_pages_->NumberOfFreeEntries(), 0);
  free_pages_->PublicFreeRanges(txn.ToFreeExtents());
  EXPECT_EQ(grow_blob->extents.extent_pid[3],
            9 + ((blob_likely_grow) ? 0 : blob->extents.special_blk.page_cnt));
  // There should be a total of 4 allocated extents
  EXPECT_EQ(free_pages_->NumberOfFreeEntries(), 4);
  // There should be 4 ranges (format: [Start PID, size]): [1, 1], [2, 2], [4,
  // 4], [8, 8]
  pageid_t start_pid;
  storage::TierList to_free_list;
...
static constexpr auto TEST_SET{[]() constexpr {
  ...
  for (int var : {0, 1, 2}) {
    for (bool likely_grow : {false, true}) {
      for (bool norm_bm : {false, true}) {
        result[idx++] = {var, likely_grow, norm_bm};
      }
    }
  }
  return result;
...
INSTANTIATE_TEST_CASE_P(TestBlobManager, TestBlobManager,
                        ::testing::ValuesIn(TEST_SET));
