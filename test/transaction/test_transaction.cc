...
TEST_F(TestTransaction, TransactionLifeTime) {
  ...
  EXPECT_EQ(logger.TotalFreeSpace(),
            LW::WAL_BUFFER_SIZE - sizeof(recovery::LogMetaEntry));
  ...
  auto thread = std::thread([&]() {
    ...
    EXPECT_EQ(x_logger.TotalFreeSpace(),
              LW::WAL_BUFFER_SIZE - sizeof(recovery::LogMetaEntry));
...
TEST_F(TestTransaction, AllocateBlobWithLogging) {
  ...
  auto blob_size = 3 * MB;
  auto random_blob =
      static_cast<u8 *>(aligned_alloc(BLK_BLOCK_SIZE, blob_size));
  for (size_t i = 0; i < blob_size; i++) {
    random_blob[i] = rand() % 255;
  }
  ...
  auto log_tx =
      reinterpret_cast<recovery::LogMetaEntry *>(&logger.wal_buffer[0]);
  ...
  for (size_t idx = 0; idx < (blob_size / FLAGS_blob_log_segment_size); idx++) {
    auto log_ent = reinterpret_cast<recovery::BlobEntry *>(
        &logger.wal_buffer[curr_w_cursor]);
    EXPECT_EQ(log_ent->type, recovery::LogEntry::Type::BLOB_ENTRY);
    EXPECT_EQ(log_ent->part_id, idx);
    EXPECT_EQ(log_ent->size,
              sizeof(recovery::BlobEntry) +
                  FLAGS_blob_log_segment_size); // WAL header + BLOB payload
