...
TEST_F(TestLogManager, NormalOperation) {
  ...
  // Submit Big Data log
  auto &dt_entry =
      logger.ReserveDataLog(LogWorker::WAL_BUFFER_SIZE - 200, 0, 0);
  logger.SubmitActiveLogEntry();
  current_log_size -= dt_entry.size;
  // Extra padding might be added to enfore memory-alignment of the log entry
  EXPECT_EQ(logger.active_log->size,
            LogWorker::WAL_BUFFER_SIZE - 200 + sizeof(DataEntry));
  EXPECT_EQ(logger.TotalFreeSpace(), current_log_size);
  EXPECT_EQ(logger.TotalFreeSpace(), logger.ContiguousFreeSpaceForNewEntry());
  ...
  auto cr_entry =
      reinterpret_cast<LogEntry *>(&logger.wal_buffer[size_of_3logs]);
  ...
  EXPECT_EQ(logger.TotalFreeSpace(),
            LogWorker::WAL_BUFFER_SIZE - commit_entry.size - cr_entry->size);
  EXPECT_EQ(logger.ContiguousFreeSpaceForNewEntry(),
            logger.gct_cursor.load() - logger.w_cursor);
}
