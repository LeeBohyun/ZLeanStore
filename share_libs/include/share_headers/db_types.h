...
struct BytesPayload {
  ...
  // Still fine to have this convenience ctor
  BytesPayload(const uint8_t *str, uint32_t len) {
    assert(len <= Size);
    std::memcpy(value, str, len);
  }

  // Let the compiler generate trivial copy operations
  BytesPayload(const BytesPayload&) = default;
  BytesPayload& operator=(const BytesPayload&) = default;

  // Comparison should be const-correct and by const-ref
