...
HybridLatchImpl<SyncStateClass>::HybridLatchImpl() {
  state_and_version_.store(SameVersionNewState(0, SyncStateClass::UNLOCKED),
...
template <class SyncStateClass>
auto HybridLatchImpl<SyncStateClass>::TryLockExclusive(u64 old_state_w_version)
    -> bool {
...
void HybridLatchImpl<SyncStateClass>::UnlockExclusive() {
  ...
  state_and_version_.store(
      NextVersionNewState(state_and_version_.load(), SyncStateClass::UNLOCKED),
      std::memory_order_release);
}
...
void HybridLatchImpl<SyncStateClass>::DowngradeLock() {
  ...
  state_and_version_.store(NextVersionNewState(state_and_version_.load(), 1),
                           std::memory_order_release);
}

template <class SyncStateClass>
auto HybridLatchImpl<SyncStateClass>::TryLockShared(u64 old_state_w_version)
    -> bool {
...
template <class SyncStateClass>
auto HybridLatchImpl<SyncStateClass>::UpgradeLock(u64 old_state_w_version)
    -> bool {
  if (LockState(old_state_w_version) != 1) {
    // Only upgrade from shared -> exclusive if there is one reader
    return false;
  }
  return state_and_version_.compare_exchange_weak(
      old_state_w_version,
      SameVersionNewState(old_state_w_version, SyncStateClass::EXCLUSIVE));
}
...
void HybridLatchImpl<SyncStateClass>::UnlockShared() {
  while (true) {
    ...
    if (state_and_version_.compare_exchange_strong(
            old_state_w_version,
