...
SharedGuard<PageClass>::SharedGuard(pageid_t pid, PageClass *ptr,
...
template <class PageClass>
auto SharedGuard<PageClass>::operator=(SharedGuard<PageClass> &&other) noexcept(
    false) -> SharedGuard & {
...
template <class PageClass> SharedGuard<PageClass>::~SharedGuard() noexcept {
  this->Unlock();
}

template <class PageClass> void SharedGuard<PageClass>::Unlock() {
  if (this->mode_ == GuardMode::SHARED) {
    this->ps_->UnlockShared();
