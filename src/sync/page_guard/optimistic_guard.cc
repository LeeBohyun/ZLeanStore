...
OptimisticGuard<PageClass>::OptimisticGuard(
    pageid_t pid, PageClass *ptr, PageState *page_state,
...
template <class PageClass>
OptimisticGuard<PageClass>::OptimisticGuard(
    OptimisticGuard<PageClass> &&other) noexcept {
...
template <class PageClass>
auto OptimisticGuard<PageClass>::operator=(
    OptimisticGuard<PageClass> &&other) noexcept(false) -> OptimisticGuard & {
  if (this->mode_ == GuardMode::OPTIMISTIC) {
    this->ValidateOrRestart();
  }
  this->page_id_ = other.page_id_;
  this->ps_ = other.ps_;
  this->page_ptr_ = other.page_ptr_;
...
template <class PageClass> void OptimisticGuard<PageClass>::Init() {
  assert(this->mode_ == GuardMode::OPTIMISTIC);
  for (auto cnt = 0;; cnt++) {
    // LOG_INFO("init pid: %lu", this->page_id_);
...
void OptimisticGuard<PageClass>::ValidateOrRestart(bool auto_moved) {
  ...
  if (PageState::Version(this->old_version_) ==
      PageState::Version(current_version)) {
