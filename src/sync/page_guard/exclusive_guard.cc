...
ExclusiveGuard<PageClass>::ExclusiveGuard(pageid_t pid, PageClass *ptr,
...
template <class PageClass>
ExclusiveGuard<PageClass>::ExclusiveGuard(
    OptimisticGuard<PageClass> &&other) noexcept(false) {
  assert(other.mode_ == GuardMode::OPTIMISTIC);
  for (auto cnt = 0;; cnt++) {
    auto &ps = other.ps_;
    u64 state_w_version = ps->StateAndVersion().load();
    if (PageState::Version(state_w_version) !=
        PageState::Version(other.old_version_)) {
...
template <class PageClass>
auto ExclusiveGuard<PageClass>::operator=(
    ExclusiveGuard<PageClass> &&other) noexcept(false) -> ExclusiveGuard & {
...
template <class PageClass> void ExclusiveGuard<PageClass>::Unlock() {
  if (this->mode_ == GuardMode::EXCLUSIVE) {
    this->ps_->UnlockExclusive();
