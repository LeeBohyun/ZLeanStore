...
void PageState::Init() {
  state_and_version_.store(SameVersionNewState(0, PageState::EVICTED),
                           std::memory_order_release);
}

auto PageState::ToString() -> std::string {
  auto v = state_and_version_.load();
  std::string result = std::string("[State: ") +
                       std::to_string(PageState::LockState(v)) +
                       std::string(", Version: ") +
...
void PageState::UnlockExclusiveAndEvict() {
  ...
  state_and_version_.store(
      NextVersionNewState(state_and_version_.load(), PageState::EVICTED),
      std::memory_order_release);
}
...
void PageState::UnlockExclusiveAndMark() {
  assert(IsExclusivelyLatched(StateAndVersion()));
  state_and_version_.store(
      NextVersionNewState(state_and_version_.load(), PageState::MARKED),
      std::memory_order_release);
}
...
auto PageState::TryLockShared(u64 old_state_w_version) -> bool {
  ...
  if (state == PageState::MARKED) {
    return state_and_version_.compare_exchange_strong(
        old_state_w_version, SameVersionNewState(old_state_w_version, 1));
  }
...
auto PageState::UnmarkAndLockExclusive(u64 old_state_w_version) -> bool {
  assert(LockState(old_state_w_version) == PageState::MARKED);
  auto new_state =
      SameVersionNewState(old_state_w_version, PageState::EXCLUSIVE);
  return state_and_version_.compare_exchange_weak(old_state_w_version,
                                                  new_state);
}
