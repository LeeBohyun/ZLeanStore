...
// -------------------------------------------------------------------------------------
BTreeNodeHeader::BTreeNodeHeader(bool is_leaf, indexid_t index)
    : is_leaf(is_leaf) {
  PageHeader::idx_id = index;
}

auto BTreeNodeHeader::HasRightNeighbor() -> bool {
  return next_leaf_node != BTreeNodeHeader::EMPTY_NEIGHBOR;
}

auto BTreeNodeHeader::HasLeftNeighbor() -> bool {
  return prev_leaf_node != BTreeNodeHeader::EMPTY_NEIGHBOR;
}

auto BTreeNodeHeader::IsLowerFenceInfinity() -> bool {
  return lower_fence.len == 0;
};

auto BTreeNodeHeader::IsUpperFenceInfinity() -> bool {
  return upper_fence.len == 0;
};

// -------------------------------------------------------------------------------------
BTreeNode::BTreeNode(bool is_leaf, indexid_t index)
    : BTreeNodeHeader(is_leaf, index) {
  dirty = true;
}
...
auto BTreeNode::FindChild(std::span<u8> search_key, leng_t &pos,
                          const ComparisonLambda &cmp) -> pageid_t {
  pos = LowerBound(search_key, cmp);
  return (pos == count) ? right_most_child : GetChild(pos);
}

auto BTreeNode::FindChild(std::span<u8> search_key, const ComparisonLambda &cmp)
    -> pageid_t {
  leng_t unused;
...
auto BTreeNode::CommonPrefix(leng_t lhs_slot, leng_t rhs_slot) -> leng_t {
  ...
  leng_t limit =
      std::min(slots[lhs_slot].key_length, slots[rhs_slot].key_length);
...
void BTreeNode::GetSeparatorKey(u8 *out_separator_key,
                                const SeparatorInfo &info) {
  std::memcpy(out_separator_key, GetPrefix(), prefix_len);
  std::memcpy(out_separator_key + prefix_len,
              GetKey(info.slot + static_cast<u8>(info.is_truncated)),
              info.len - prefix_len);
}
...
auto BTreeNode::ToString() -> std::string {
  ...
  if (is_leaf) {
    extra_metadata = "PrevSibling(" + std::to_string(prev_leaf_node) + ")";
  ...
  return fmt::format("<\n\tHeader: idx_id({:d}) - p_gsn({:d})\n\tMetadata: "
                     "IsLeaf({}) - Count({:d}) - Free space({:d}) - {}"
                     "\n\tPrefix({}) - LowerFences({}) - "
                     "UpperFences({})\n\tKeys: [{}]\n\tPayloads: [{}]\n>",
                     idx_id, p_gsn, is_leaf, count, FreeSpaceAfterCompaction(),
                     extra_metadata, prefix, GetLowerFence(), GetUpperFence(),
                     all_keys, payload);
}

// -------------------------------------------------------------------------------------
auto BTreeNode::GetLowerFence() -> std::span<u8> {
  return {Ptr() + lower_fence.offset, lower_fence.len};
}

auto BTreeNode::GetUpperFence() -> std::span<u8> {
  return {Ptr() + upper_fence.offset, upper_fence.len};
}
...
void BTreeNode::SetFences(std::span<u8> lower, std::span<u8> upper,
  ...
  if (cmp.op == ComparisonOperator::MEMCMP) {
    ...
    for (prefix_len = 0; (prefix_len < std::min(lower.size(), upper.size())) &&
                         (lower[prefix_len] == upper[prefix_len]);
         prefix_len++) {
    }
  }
}
...
auto BTreeNode::FreeSpace() -> leng_t {
  // zeroing out free spaces for better compression ratio
  memset(reinterpret_cast<u8 *>(slots + count), 0,
         (data_offset - (reinterpret_cast<u8 *>(slots + count) - Ptr())));
  /* end */
  return data_offset - (reinterpret_cast<u8 *>(slots + count) - Ptr());
}

auto BTreeNode::FreeSpaceAfterCompaction() -> leng_t {
  return PAGE_SIZE - (reinterpret_cast<u8 *>(slots + count) - Ptr()) -
         space_used;
}

auto BTreeNode::SpaceRequiredForKV(leng_t key_len, leng_t payload_len)
    -> leng_t {
...
// -------------------------------------------------------------------------------------
auto BTreeNode::GetKey(leng_t slot_id) -> u8 * {
  return Ptr() + slots[slot_id].offset;
}

auto BTreeNode::GetPayload(leng_t slot_id) -> std::span<u8> {
  return {Ptr() + slots[slot_id].offset + slots[slot_id].key_length,
          slots[slot_id].payload_length};
}

auto BTreeNode::GetChild(leng_t slot_id) -> pageid_t {
...
void BTreeNode::MakeHint() {
  ...
  for (size_t idx = 0; idx < BTreeNode::HINT_COUNT; idx++) {
    hints[idx] = slots[dist * (idx + 1)].head;
  }
}
...
void BTreeNode::UpdateHint(leng_t slot_id) {
  auto dist = count / (BTreeNode::HINT_COUNT + 1);
  auto begin = 0;
  if ((count > BTreeNode::HINT_COUNT * 2 + 1) &&
      (((count - 1) / (BTreeNode::HINT_COUNT + 1)) == dist) &&
      ((slot_id / dist) > 1)) {
    begin = (slot_id / dist) - 1;
  }

  for (size_t idx = begin; idx < BTreeNode::HINT_COUNT; idx++) {
    hints[idx] = slots[dist * (idx + 1)].head;
  }
}

void BTreeNode::SearchHint(const std::array<u8, 4> &key_head,
                           leng_t &lower_bound, leng_t &upper_bound) {
  if (count > BTreeNode::HINT_COUNT * 2) {
    leng_t dist = upper_bound / (BTreeNode::HINT_COUNT + 1);
    leng_t lower_hint_p;
    leng_t upper_hint_p;
    for (lower_hint_p = 0; lower_hint_p < BTreeNode::HINT_COUNT;
         lower_hint_p++) {
      if (std::memcmp(hints[lower_hint_p].data(), key_head.data(), 4) >= 0) {
        break;
      }
    }

    for (upper_hint_p = lower_hint_p; upper_hint_p < BTreeNode::HINT_COUNT;
         upper_hint_p++) {
      if (std::memcmp(hints[upper_hint_p].data(), key_head.data(), 4) != 0) {
        break;
      }
    }

    lower_bound = lower_hint_p * dist;
    if (upper_hint_p < BTreeNode::HINT_COUNT) {
      upper_bound = (upper_hint_p + 1) * dist;
    }
  }
}

// -------------------------------------------------------------------------------------
auto BTreeNode::LowerBound(std::span<u8> search_key, bool &exact_found,
                           const ComparisonLambda &cmp) -> leng_t {
  exact_found = false;

  int ret = 0;
  if (cmp.op == ComparisonOperator::MEMCMP) {
    // evaluate if the search key is within the prefix of the current node (i.e.
    // prefix of all records inside this node)
    ret =
        cmp.func(search_key.data(), GetPrefix(),
                 std::min(search_key.size(), static_cast<size_t>(prefix_len)));
    if (ret < 0) {
      // LOG_INFO("ret < 0");
      return 0;
    }
  ...
  if (search_key.size() < prefix_len) {
    LOG_INFO("search_key.size() < prefix_len");
    return 0;
  }
  ...
  while (lower < upper) {
    ...
    if (std::memcmp(key_head.data(), slots[mid].head.data(), 4) < 0) {
      ...
      ret = cmp.func(
          key, GetKey(mid),
          std::min(key_length, static_cast<size_t>(slots[mid].key_length)));
      if (ret < 0) {
        upper = mid;
      } else if (ret > 0) {
        lower = mid + 1;
...
auto BTreeNode::LowerBound(std::span<u8> search_key,
                           const ComparisonLambda &cmp) -> leng_t {
  bool ignore;
...
// -------------------------------------------------------------------------------------
void BTreeNode::StoreRecordData(leng_t slot_id, std::span<u8> full_key,
...
void BTreeNode::StoreRecordDataWithoutPrefix(leng_t slot_id,
                                             std::span<u8> key_no_prefix,
                                             std::span<const u8> payload) {
  ...
  slots[slot_id] =
      (PageSlot){data_offset, static_cast<leng_t>(key_no_prefix.size()),
                 static_cast<leng_t>(payload.size()),
                 BTreeNode::GetHead(key, key_no_prefix.size())};
 ...
 */
void BTreeNode::InsertKeyValue(std::span<u8> key, std::span<const u8> payload,
                               const ComparisonLambda &cmp) {
...
auto BTreeNode::RemoveKey(std::span<u8> key, const ComparisonLambda &cmp)
    -> bool {
  bool found;
  auto slot_id = LowerBound(key, found, cmp);
...
void BTreeNode::CopyKeyValue(BTreeNode *dst, leng_t src_slot, leng_t dst_slot) {
  ...
  std::memcpy(full_key + prefix_len, GetKey(src_slot),
              slots[src_slot].key_length);
  dst->StoreRecordData(dst_slot, {full_key, full_key_length},
                       GetPayload(src_slot));
  dst->count++;
}

void BTreeNode::CopyNodeContent(BTreeNode *dst, BTreeNode *src) {
  std::memset(reinterpret_cast<u8 *>(dst), 0, sizeof(BTreeNode));
  std::memcpy(reinterpret_cast<u8 *>(dst), reinterpret_cast<u8 *>(src),
              sizeof(BTreeNode));
  if (!dst->IsInner() && !src->IsInner()) {
    dst->FreeSpace();
    src->FreeSpace();
  }
}

void BTreeNode::CopyKeyValueRange(BTreeNode *dst, leng_t dst_slot,
                                  leng_t src_slot, leng_t src_count) {
  // prefix grows
  assert(std::memcmp(GetPrefix(), dst->GetPrefix(),
                     std::min(prefix_len, dst->prefix_len)) == 0);
  if (prefix_len <= dst->prefix_len) {
    ...
    for (auto idx = 0; idx < src_count; idx++) {
      ...
      dst->StoreRecordDataWithoutPrefix(
          dst_slot + idx,
    ...
    for (auto idx = 0; idx < src_count; idx++) {
      CopyKeyValue(dst, src_slot + idx, dst_slot + idx);
    }
  }
  assert((dst->Ptr() + dst->data_offset) >=
         reinterpret_cast<u8 *>(dst->slots + dst->count));
  /* lia for zeroing out*/
  if (!IsInner()) {
    FreeSpace();
  }
...
// -------------------------------------------------------------------------------------
auto BTreeNode::FindSeparator(bool append_bias, const ComparisonLambda &cmp)
    -> SeparatorInfo {
  assert(count > 1);
  if (IsInner()) {
    ...
    return SeparatorInfo{
        static_cast<leng_t>(prefix_len + slots[slot_id].key_length), slot_id,
        false};
  }
  ...
  if (append_bias) {
    ...
    if (best_prefix_len != CommonPrefix(upper - 1, 0)) {
      for (best_slot = lower + 1;
           (best_slot < upper) &&
           (CommonPrefix(best_slot, 0) == best_prefix_len);
           best_slot++) {
      }
    }
  } else {
  ...
  if (cmp.op == ComparisonOperator::MEMCMP) {
    ...
    if ((best_slot + 1 < count) && (slots[best_slot].key_length > common) &&
      ...
      return SeparatorInfo{static_cast<leng_t>(prefix_len + common + 1),
                           best_slot, true};
    }
  }

  return SeparatorInfo{
      static_cast<leng_t>(prefix_len + slots[best_slot].key_length), best_slot,
      false};
}

void BTreeNode::SplitNode(
    BTreeNode *parent, BTreeNode *node_right, pageid_t left_pid,
    pageid_t right_pid, // left_pid == this right_pid = new_child
  ...
  if (this->is_leaf) {
    if (this->IsUpperFenceInfinity()) {
      Ensure(!this->HasRightNeighbor());
    }

    if (this->IsLowerFenceInfinity()) {
      Ensure(!this->HasLeftNeighbor());
    }
  }
  ...
  if (old_parent_slot == parent->count) {
    ...
    if (parent->GetChild(old_parent_slot) != left_pid) {
      LOG_INFO("left_pid: %lu right_pid: %lu", left_pid, right_pid);
    }
    assert(parent->GetChild(old_parent_slot) == left_pid);
    std::memcpy(parent->GetPayload(old_parent_slot).data(), &right_pid,
                sizeof(pageid_t));
  }
  parent->InsertKeyValue(
      sep_key, {reinterpret_cast<const u8 *>(&left_pid), sizeof(pageid_t)},
      cmp);

  // update content of two child nodes
  if (is_leaf) {
    CopyKeyValueRange(node_left, 0, 0, separator_slot + 1);
    CopyKeyValueRange(node_right, 0, node_left->count,
                      count - node_left->count);
    node_left->prev_leaf_node = this->prev_leaf_node;
    node_left->next_leaf_node = right_pid;
    node_right->prev_leaf_node = left_pid;
    node_right->next_leaf_node = this->next_leaf_node;
    Ensure(node_right->prev_leaf_node == left_pid);
    Ensure(node_left->next_leaf_node == right_pid);
    ...
    CopyKeyValueRange(node_right, 0, node_left->count + 1,
                      count - node_left->count - 1);
 ...
 */
auto BTreeNode::MergeNodes(leng_t left_slot_id, BTreeNode *parent,
                           BTreeNode *right, const ComparisonLambda &cmp)
    -> bool {
  BTreeNode tmp(is_leaf, idx_id);
  ...
  if (is_leaf) {
    ...
    auto space_upper_bound =
        ...
        (prefix_len - tmp.prefix_len) *
            count + //  grow from prefix compression for left node
        (right->prefix_len - tmp.prefix_len) *
            right->count + // same as above for right node
    ...
    tmp.prev_leaf_node = this->prev_leaf_node;

  } else {
    assert((right->IsInner()) && (parent->IsInner()));
    // calculate the upper bound on space used of the new node
    auto extra_key_len =
        parent->prefix_len + parent->slots[left_slot_id].key_length;
    auto space_upper_bound =
        sizeof(BTreeNodeHeader) + // size of BTreeNodeHeader (i.e. metadata)
        space_used + right->space_used + // size of all keys + their payload
        (prefix_len - tmp.prefix_len) *
    ...
    std::memcpy(parent->GetPayload(left_slot_id).data(), &right_most_child,
                sizeof(pageid_t));
...
// -------------------------------------------------------------------------------------
auto BTreeNode::FindChildWithBlobKey(const blob::BlobLookupKey &key,
                                     leng_t &pos, const ComparisonLambda &cmp)
    -> pageid_t {
  bool unused;
...
auto BTreeNode::LowerBoundWithBlobKey(const blob::BlobLookupKey &key,
                                      bool &exact_found,
                                      const ComparisonLambda &cmp) -> leng_t {
  exact_found = false;

  /**
   * @brief This should only be called in Blob State indexes,
