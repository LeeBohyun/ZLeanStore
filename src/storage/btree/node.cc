...
auto BTreeNode::FindChild(std::span<u8> search_key,
                          const ComparisonLambda &cmp) -> pageid_t {
  leng_t unused;
...
auto BTreeNode::SpaceRequiredForKV(leng_t key_len,
                                   leng_t payload_len) -> leng_t {
...
auto BTreeNode::RemoveKey(std::span<u8> key,
...
// -------------------------------------------------------------------------------------
auto BTreeNode::FindSeparator(bool append_bias,
                              const ComparisonLambda &cmp) -> SeparatorInfo {
  assert(count > 1);
  if (IsInner()) {
