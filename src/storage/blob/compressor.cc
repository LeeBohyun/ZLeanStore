...
void BlobCompressor::CompressBlob(std::span<const u8> blob) {
  ...
  case 1:
    FSSTCompressBlob(blob);
    break;
  case 2:
    ZSTDCompressBlob(blob);
    break;
  case 3:
    LZ4CompressBlob(blob);
    break;
  default:
    UnreachableCode();
  }
...
void BlobCompressor::FSSTCompressBlob(const std::span<const u8> &blob) {
  ...
  auto ret = fsst_compress(fsst, 1, blob_size, blob_ptr, max_buffer_sz - size,
                           &buffer[size], &compressed_size, unused);
  Ensure(ret >= 1);
...
void BlobCompressor::ZSTDCompressBlob(const std::span<const u8> &blob) {
  ...
  size = ZSTD_compress(buffer, max_buffer_sz, blob.data(), blob.size(),
                       FLAGS_zstd_compress_level);
...
void BlobCompressor::LZ4CompressBlob(const std::span<const u8> &blob) {
  ...
  size = LZ4_compress_fast(reinterpret_cast<const char *>(blob.data()),
                           reinterpret_cast<char *>(buffer), blob.size(),
