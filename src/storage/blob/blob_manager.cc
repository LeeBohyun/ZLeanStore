...
#define ALLOCATE_REMAIN_CONTENT(idx, is_special_block)                         \
  ({                                                                           \
    auto remaining_bytes = std::min(ExtentList::ExtentSize(idx) * PAGE_SIZE,   \
                                    payload.size() - offset);                  \
    auto pid = out_blob->extents.extent_pid[idx] = buffer_->AllocExtent(       \
        (idx),                                                                 \
        (is_special_block) ? BlobState::PageCount(remaining_bytes) : 0);       \
    if (FLAGS_blob_normal_buffer_pool) {                                       \
      buffer_->ChunkOperation(                                                 \
          pid, remaining_bytes, [&](u64 off, std::span<u8> page_addr) {        \
            std::memcpy(page_addr.data(), payload.data() + offset + off,       \
                        page_addr.size());                                     \
          });                                                                  \
    } else {                                                                   \
      std::memcpy(buffer_->ToPtr(pid), payload.data() + offset,                \
                  remaining_bytes);                                            \
    }                                                                          \
...
#define PREPARE_FREE_SPECIAL_BLK(blob)                                         \
  ({                                                                           \
    ...
    (blob)->extents.special_blk.SplitToExtents(                                \
        [&](pageid_t start_pid, extidx_t index) {                              \
          buffer_->GetFreePageManager()->PrepareFreeTier(start_pid, index);    \
          pid += ExtentList::ExtentSize(index);                                \
        });                                                                    \
    Ensure(pid == (blob)->extents.special_blk.start_pid +                      \
                      (blob)->extents.special_blk.page_cnt);                   \
  })

#define SHA2_CALC_LP(extent)                                                   \
  ({                                                                           \
    auto remaining_bytes =                                                     \
        std::min(out_blob->blob_size - offset, (extent).page_cnt * PAGE_SIZE); \
    if (FLAGS_blob_normal_buffer_pool) {                                       \
      buffer_->ChunkOperation((extent).start_pid, remaining_bytes,             \
                              [&](u64, std::span<u8> page) {                   \
                                EVP_DigestUpdate(BlobState::sha_context.get(), \
                                                 page.data(), page.size());    \
                              });                                              \
    } else {                                                                   \
      EVP_DigestUpdate(BlobState::sha_context.get(),                           \
                       buffer_->ToPtr((extent).start_pid), remaining_bytes);   \
    }                                                                          \
    offset += remaining_bytes;                                                 \
  })
 ...
 *
 * Every large page of `out_to_write_lps` is either a portion or the whole
 * extent of the corresponding extent in `out_to_evict_ets` That is, assume the
 * large page `lp = out_to_write_lps[i]` and the respective extent `ext =
 * out_to_evict_ets[i]` then, `lp.start_pid >= ext.start_pid && lp.start_pid +
 * lp.page_cnt <= ext.start_pid + ext.page_cnt`
 *
 * @param out_blob          The Blob State ptr
 * @param ext_id            The index of the extent in `out_blob->extents`
 * @param start_pid         The Start PID of the dirty pages, not Start PID of
 * the extent
 * @param pg_cnt            Number of dirty pages, not size of the extent
 * @param out_to_write_lps  A vector containing all large pages to be written
 * out in Group Commit
 * @param out_to_evict_ets  A vector containing all extents to be logically
 * evicted by Buffer Manager
 */
#define MARK_EXTENT_EVICT(out_blob, ext_id, start_pid, pg_cnt,                 \
                          out_to_write_lps, out_to_evict_ets)                  \
  ({                                                                           \
    pageid_t page_id = (out_blob)->extents.extent_pid[(ext_id)];               \
    Ensure(                                                                    \
        (page_id <= (start_pid)) &&                                            \
        ((start_pid) + (pg_cnt) <= page_id + ExtentList::ExtentSize(ext_id))); \
...
thread_local std::array<u8, BlobState::MallocSize(ExtentList::EXTENT_CNT_MASK)>
    BlobManager::blob_state_storage;

// -------------------------------------------------------------------------------------
PageAliasGuard::PageAliasGuard(buffer::BufferManager *buffer,
                               const BlobState &blob, u64 required_load_size)
    : buffer_(buffer) {
  ...
  if (FLAGS_blob_normal_buffer_pool) {
    ...
    u64 offset = 0;
    size_t idx = 0;
    for (; (idx < blob.extents.NumberOfExtents()) &&
           (offset < required_load_size);
         idx++) {
      auto copy_size = std::min(required_load_size - offset,
                                ExtentList::ExtentSize(idx) * PAGE_SIZE);
      buffer->ChunkOperation(blob.extents.extent_pid[idx], copy_size,
                             [&](u64 off, std::span<u8> payload) {
                               std::memcpy(&ptr_[offset + off], payload.data(),
                                           payload.size());
                             });
      offset += copy_size;
    }
    if (blob.extents.special_blk.in_used && offset < required_load_size) {
      ...
      auto copy_size = std::min(required_load_size - offset,
                                blob.extents.special_blk.page_cnt * PAGE_SIZE);
      buffer->ChunkOperation(blob.extents.special_blk.start_pid, copy_size,
                             [&](u64 off, std::span<u8> payload) {
                               std::memcpy(&ptr_[offset + off], payload.data(),
                                           payload.size());
                             });
  ...
  size_t idx = 0;
  for (; (idx < blob.extents.NumberOfExtents()) &&
         (alias_size < required_load_size);
       idx++) {
    auto pg_cnt = ExtentList::ExtentSize(idx);
#ifdef ENABLE_EXMAP
    buffer->exmap_interface_[worker_thread_id]->iov[idx].page =
        blob.extents.extent_pid[idx];
  ...
  if (blob.extents.special_blk.in_used && alias_size < required_load_size) {
    Ensure(idx++ == blob.extents.NumberOfExtents());
#ifdef ENABLE_EXMAP
    buffer->exmap_interface_[worker_thread_id]->iov[idx - 1].page =
        blob.extents.special_blk.start_pid;
    buffer->exmap_interface_[worker_thread_id]->iov[idx - 1].len =
        blob.extents.special_blk.page_cnt;
#endif
...
BlobManager::BlobManager(buffer::BufferManager *buffer_manager)
    : buffer_(buffer_manager) {}

/**
 * @brief Set all extents of the given special blocks for eviction
 */
void BlobManager::MarkTailExtentForEviction(const TailExtent &special_blk,
                                            LargePageList &out_to_write_lps,
                                            LargePageList &out_to_evict_ets) {
  if (!special_blk.in_used) {
    return;
  }
...
void BlobManager::FreshBlobAllocation(std::span<const u8> payload,
                                      BlobState *out_blob, bool likely_grow,
                                      LargePageList &out_to_write_lps,
                                      LargePageList &out_to_evict_ets) {
  ...
  // If the payload fits 100% into our tiering allocation, there will be no
  // difference between
  //  fixed-size blob allocation vs grow-likely blob allocation
  if (required_page_cnt ==
      ExtentList::TotalSizeExtents(required_no_extents - 1)) {
    likely_grow = true;
  }
  ...
  for (u8 idx = 0; idx < required_no_extents; idx++) {
    ALLOCATE_REMAIN_CONTENT(idx,
                            (!likely_grow) && (idx == required_no_extents - 1));
  }

  // If this is a fixed allocation, reduce extent_cnt and set the corresponding
  // `special_blk`
  if (!likely_grow) {
    // If required_no_extents == 1, then required_page_cnt ==
    // ExtentList::TotalSizeExtents(required_no_extents - 1)
    //  i.e. likely_grow will be reset to true at the start of this function
    Ensure(required_no_extents > 1);
    auto last_blk_size = required_page_cnt -
                         ExtentList::TotalSizeExtents(required_no_extents - 2);
  ...
  for (size_t idx = 0; idx < out_blob->extents.extent_cnt; idx++) {
    ...
    MARK_EXTENT_EVICT(out_blob, idx, pid,
                      std::min(ExtentList::ExtentSize(idx), remaining_pages),
                      out_to_write_lps, out_to_evict_ets);
    remaining_pages -= ExtentList::ExtentSize(idx);
  }
  MarkTailExtentForEviction(out_blob->extents.special_blk, out_to_write_lps,
                            out_to_evict_ets);
}

void BlobManager::ExtendExistingBlob(std::span<const u8> payload,
                                     BlobState *out_blob,
                                     LargePageList &out_to_write_lps,
                                     LargePageList &out_to_evict_ets) {
  // Calculate number of extents to be allocated
  auto prev_no_extents = ExtentList::NoSpanExtents(out_blob->PageCount());
  auto required_page_cnt =
      BlobState::PageCount(out_blob->blob_size + payload.size());
  auto required_no_extents = ExtentList::NoSpanExtents(required_page_cnt);
  Ensure(required_no_extents > 0);
  ...
  for (auto idx = prev_no_extents; idx < required_no_extents; idx++) {
    ...
    MARK_EXTENT_EVICT(
        out_blob, idx, static_cast<pageid_t>(out_blob->extents.extent_pid[idx]),
 ...
 */
void BlobManager::LoadBlobContent(const BlobState *blob,
                                  u64 required_load_size) {
  ...
  // If the load size still not meet the requirement, then we load the special
  // block
  if (load_size < required_load_size) {
    Ensure(blob->extents.special_blk.in_used);
    blob->extents.special_blk.SplitToExtents([&](pageid_t start_pid,
                                                 extidx_t index) {
      extent_loaded.add(start_pid);
      to_read_extents.emplace_back(start_pid, ExtentList::ExtentSize(index));
    });
    Ensure(load_size + blob->extents.special_blk.page_cnt * PAGE_SIZE >=
           required_load_size);
  }

  // Trigger the necessary read
  if (!to_read_extents.empty()) {
    buffer_->ReadExtents(to_read_extents);
  }
}
 ...
 */
auto BlobManager::WriteNewDataToLastExtent(transaction::Transaction &txn,
  ...
  if (remain_free_bytes > 0) {
    // If the new payload fits into remaining space of the last extent, write it
    // there
    auto last_idx = blob->extents.NumberOfExtents() - 1;
    auto last_alloc_sz = ExtentList::ExtentSize(last_idx) * PAGE_SIZE;

    // Upgrade Lock of the last Extent from SHARED -> EXCLUSIVE, and remove it
    // from the `extent_loaded` state
    Ensure(extent_loaded.contains(blob->extents.extent_pid[last_idx]));
    extent_loaded.remove(blob->extents.extent_pid[last_idx]);
    buffer_->GetPageState(blob->extents.extent_pid[last_idx])
        .ForceUpgradeLock();

    /**
     * @brief Determine the dirty pid, calculate by X + Y - Z, where the three
     * variables are:
     ...
     */
    auto pid =
        blob->extents.extent_pid[last_idx] + ExtentList::ExtentSize(last_idx) -
        static_cast<u64>(
            std::ceil(static_cast<float>(remain_free_bytes) / PAGE_SIZE));
    ...
    if (remain_free_bytes % PAGE_SIZE > 0) {
      auto &log_entry = txn.LogWorker().ReservePageImageLog(
          PAGE_SIZE - (remain_free_bytes % PAGE_SIZE), pid,
          0); // offset parameter 0 dummy value
      std::memcpy(log_entry.payload, buffer_->ToPtr(pid),
                  PAGE_SIZE - (remain_free_bytes % PAGE_SIZE));
      txn.LogWorker().SubmitActiveLogEntry();
    }

    // Only write to the last remaining free bytes of the last extent and mark
    // the Extent for evict
    // TODO(Duy): Add overhead of normal buffer pool here
    auto write_to_addr = reinterpret_cast<u8 *>(buffer_->ToPtr(
                             blob->extents.extent_pid[last_idx])) +
                         last_alloc_sz - remain_free_bytes;
    ...
    auto evict_size =
        static_cast<u64>(std::ceil(static_cast<float>(write_size) / PAGE_SIZE));
    Ensure(pid == blob->extents.extent_pid[last_idx] +
                      ExtentList::ExtentSize(last_idx) - evict_size);
    MARK_EXTENT_EVICT(blob, last_idx, pid, evict_size,
                      txn.ToFlushedLargePages(), txn.ToEvictedExtents());
  }
 ...
 */
auto BlobManager::MoveTailExtent(transaction::Transaction &txn,
                                 std::span<const u8> payload, BlobState *blob)
    -> u64 {
  u64 write_size = 0;

  // 1. Allocate a new extent to the end of the extent sequence
  auto last_idx = blob->extents.extent_cnt++;
  auto pid = blob->extents.extent_pid[last_idx] =
      buffer_->AllocExtent(last_idx, 0);

  // 2. Move the content of special block to the new allocated extent
  auto special_blk_data_sz =
      blob->blob_size - ExtentList::TotalSizeExtents(last_idx - 1) * PAGE_SIZE;
  Ensure(special_blk_data_sz <= blob->extents.special_blk.page_cnt * PAGE_SIZE);
  auto from_addr = reinterpret_cast<u8 *>(
      buffer_->ToPtr(blob->extents.special_blk.start_pid));
  if (FLAGS_blob_normal_buffer_pool) {
    buffer_->ChunkOperation(
        pid, special_blk_data_sz, [&](u64 offset, std::span<u8> page_addr) {
  ...
  // TODO(Duy): Add overhead of normal buffer pool here
  write_size =
      std::min(payload.size(), ExtentList::ExtentSize(last_idx) * PAGE_SIZE -
                                   special_blk_data_sz);
  std::memcpy(reinterpret_cast<u8 *>(buffer_->ToPtr(pid)) + special_blk_data_sz,
              payload.data(), write_size);
  blob->blob_size += write_size;

  // 4. Mark the last extent for Evict()
  MARK_EXTENT_EVICT(blob, last_idx, pid, ExtentList::ExtentSize(last_idx),
                    txn.ToFlushedLargePages(), txn.ToEvictedExtents());

  // 5. Remove all the splitted extents of special block from `extent_loaded`
  // status
  blob->extents.special_blk.SplitToExtents(
      [&](pageid_t pid, [[maybe_unused]] extidx_t) {
        extent_loaded.remove(pid);
      });

  // 6. Mark the special block as unused and add the extents to the reusable
  // list
...
/**
 * @brief Allocate new BLOB and mark the new content for async-write in Group
 * Commit For now, only support:
 * - Allocating a whole new blob
 * - Appending content to an existing blob
 *
 * *NOTE*: Do not mark this newly allocated Blob as full load, because if we do
 * it, then during transaction commit, those db pages will be UnfixShare-ed,
 * causing undefined behavior Only committed Blobs will be in the
 * `extent_loaded` life cycle
 *
 * @param payload           Payload of this Blob
 * @param prev_blob         The BlobState refers to prev blob, nullptr if this
 * is a new allocation
 * @param likely_grow       Whether it's likely that this new allocated Blob
 * will grow in the future If it isn't, then we will allocate a special block
 * (if necessary) to store its tail data
 */
auto BlobManager::AllocateBlob(std::span<const u8> payload,
                               const BlobState *prev_blob, bool likely_grow)
    -> BlobState * {
  // Run-time environment
  blob_state_storage = {0};
  auto out_blob = reinterpret_cast<BlobState *>(blob_state_storage.data());
  ...
  // If this is a growing operator, load the content of previous blob into
  // memory We only need the content of all previous extents, i.e. don't need
  // them in contiguous memory
  if (prev_blob != nullptr) {
    LoadBlobContent(prev_blob, prev_blob->blob_size);
  }
   ...
   *  - User specifies that the new blob is unlikely to grow, i.e. likely_grow
   * == false
   * - If this is an append allocation (prev_blob != nullptr), it is ridiculous
   * that it won't be appended in the future
   * - If FLAGS_blob_tail_extent is disabled, turn off the special_block feature
   * as well
   */
  out_blob->extents.special_blk.in_used = false;
  if ((prev_blob != nullptr) || (!FLAGS_blob_tail_extent)) {
    likely_grow = true;
  }

  // Prepare blob data in the buffer manager
  if (prev_blob == nullptr) {
    FreshBlobAllocation(payload, out_blob, likely_grow,
                        current_txn.ToFlushedLargePages(),
    ...
    if (prev_blob->extents.special_blk.in_used) {
      write_sz_to_remaining_pages =
          MoveTailExtent(current_txn, payload, out_blob);
    } else {
      write_sz_to_remaining_pages =
          WriteNewDataToLastExtent(current_txn, payload, out_blob);
    }

    // If the new payload is written out fully, then there is no need to extend
    // the existing blob
    if (payload.size() > write_sz_to_remaining_pages) {
      // Otherwise, extend prev blob with un-written new data
      auto to_write_payload =
          std::span<const u8>{payload.data() + write_sz_to_remaining_pages,
                              payload.size() - write_sz_to_remaining_pages};
      ExtendExistingBlob(to_write_payload, out_blob,
                         current_txn.ToFlushedLargePages(),
                         current_txn.ToEvictedExtents());
    }
  }

  // Calculate SHA-256 value for the Blob State
  EVP_DigestInit_ex(BlobState::sha_context.get(), EVP_sha256(), nullptr);
  auto offset = 0UL;
  for (auto &extent : out_blob->extents) {
    SHA2_CALC_LP(extent);
  }
  if (offset < out_blob->blob_size) {
    // If we haven't calculated SHA-256 for this BLOB, this means we have the
    // special block
...
/**
 * @brief Remember to remove all references to this BlobHandler first before
 * calling this func
 *
 * TODO(Duy): Should we somehow X-lock this BlobState before removing it?
 * Probably we need Concurrency Control
 */
void BlobManager::RemoveBlob(const BlobState *blob) {
  ...
  for (size_t idx = 0; idx < blob->extents.NumberOfExtents(); idx++) {
    buffer_->GetFreePageManager()->PrepareFreeTier(
        blob->extents.extent_pid[idx], idx);
  }
  if (blob->extents.special_blk.in_used) {
    PREPARE_FREE_SPECIAL_BLK(blob);
  }
}

void BlobManager::LoadBlob(const BlobState *blob, u64 required_load_size,
                           const BlobCallbackFunc &cb) {
  // Don't read more the the capacity of the Blob
  if (required_load_size > blob->blob_size || required_load_size == 0) {
    required_load_size = blob->blob_size;
  }
...
void BlobManager::UnloadAllBlobs() {
  // Unfix all Blobs
  for (const auto &extent_pid : extent_loaded) {
    buffer_->UnfixShare(extent_pid);
  }
  extent_loaded.clear();
}

// -------------------------------------------------------------------------------------
auto BlobManager::BlobStateCompareWithString(const void *a, const void *b)
    -> int {
  ...
  // 1. Compare Prefix
  int ret =
      std::memcmp(lhs->blob_prefix, rhs->blob.data(), BlobState::PREFIX_LENGTH);
  if (ret != 0 || b_length < BlobState::PREFIX_LENGTH) {
    return ret;
  }

  // 2. Evaluate SHA-256 computed value
  if (std::memcmp(lhs->sha2_val, rhs->sha2_digest,
                  BlobState::SHA256_DIGEST_LENGTH) == 0) {
    return 0;
  }
  ...
  for (u8 idx = 0; idx < lhs->extents.extent_cnt; idx++) {
    if (!extent_loaded.contains(lhs->extents.extent_pid[idx])) {
      ...
      buffer_->ReadExtents({LargePage(lhs->extents.extent_pid[idx],
    ...
    // TODO(Duy): Add overhead of normal Buffer Pool here
    ret = std::memcmp(buffer_->ToPtr(lhs->extents.extent_pid[idx]),
                      rhs->blob.data() + offset, to_compare_size);
    if (ret != 0) {
      return ret;
    }
    offset += extent_size;
...
auto BlobManager::BlobStateComparison(const void *a, const void *b) -> int {
  ...
  if (std::memcmp(lhs->sha2_val, rhs->sha2_val,
                  BlobState::SHA256_DIGEST_LENGTH) == 0) {
    return 0;
  }
  ...
  // 2. Compare Prefix
  int ret =
      std::memcmp(lhs->blob_prefix, rhs->blob_prefix, BlobState::PREFIX_LENGTH);
  if (ret != 0) {
    return ret;
  }
  ...
  u64 prefix_size = 0;
  for (u8 idx = 0;
       idx < std::min(lhs->extents.extent_cnt, rhs->extents.extent_cnt);
       idx++) {
    ...
    // TODO(Duy): Add overhead of normal Buffer Pool here
    ret = std::memcmp(buffer_->ToPtr(lhs->extents.extent_pid[idx]),
                      buffer_->ToPtr(rhs->extents.extent_pid[idx]),
                      ExtentList::ExtentSize(idx) * PAGE_SIZE);
    if (ret != 0) {
      return ret;
    }
  ...
  if (std::min(lhs->blob_size, rhs->blob_size) > prefix_size) {
    ...
    LoadBlob(lhs, load_size, [&](std::span<const u8> lhs_payload) {
      LoadBlob(rhs, load_size, [&](std::span<const u8> rhs_payload) {
        ret = std::memcmp(
            lhs_payload.data() + prefix_size, rhs_payload.data() + prefix_size,
