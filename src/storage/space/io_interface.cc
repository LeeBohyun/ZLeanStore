...
IOInterface::IOInterface(int blockfd, Page *virtual_mem, u64 virtual_cnt,
    ...
    : blockfd_(blockfd), virtual_cnt_(virtual_cnt), virtual_mem_(virtual_mem),
      io_backend_(blockfd, sm, PID2Offset_table, storage_space),
      doublewrite_(blockfd, sm->max_userspace_capacity_), sm_(sm),
      PID2Offset_table_(PID2Offset_table), sspace_(storage_space) {
  Construction();
}
...
void IOInterface::ReadPagesOutOfPlace(std::vector<pageid_t> &to_read) {
  ...
  io_backend_.ReadPagesOutOfPlace(r_offsets, r_buffers, false);
...
void IOInterface::DecompressPage(char *rbuffer, storage::Page *page,
  ...
  case 0: // LZ4
  {
    ...
    if (temp_decomp_sz != PAGE_SIZE) {
      LOG_ERROR("LZ4 decompression failed: Expected: %lu, Got: %d", PAGE_SIZE,
                temp_decomp_sz);
      // LOG_INFO(
      //     "wid: %d pid: %lu decompress size: %d compressed size: %lu offset:
      //     "
      //     "%lu start_ptr: %lu",
...
u16 IOInterface::CompressPage(Page *page, char *wbuffer, u64 start_ptr) {
  ...
  case 1: // Zstd
    ...
    if (comp_sz < PAGE_SIZE) {
      ...
      // data)
      LOG_INFO("Zstd compression did not reduce size, comp_sz: %d", comp_sz);
    }
    break;
  case 2: // Zlib
  {
...
void IOInterface::ResetGCWriteBuffer(u32 cached_cnt) {
...
void IOInterface::ResetWBufferPerGroup(u32 page_cnt) {
...
u16 IOInterface::AddPageToWriteBuffer(storage::Page *page, pageid_t pid,
  ...
  if (FLAGS_use_compression) {
    comp_sz = CompressPage(page, wbuffer_.buffer, cur_write_size);
    Ensure(comp_sz < PAGE_SIZE);
  } else {
...
u16 IOInterface::AddPageToGCWriteBuffer(storage::Page *page, pageid_t pid,
  ...
  if (FLAGS_use_compression) {
    ...
    comp_sz = CompressPage(page, wbuffer_gc_.buffer, cur_write_size);
    Ensure(comp_sz < PAGE_SIZE);
  } else {
...
void IOInterface::FlushWriteBufferPerGroup(u32 group_idx, blockid_t cur_bid,
  ...
  if (cur_bid == block_cnt_) {
    ...
    while (bid >= sspace_->block_cnt_) {
      bid = sm_->SelectOpenBlockID(wbuffer_group_[group_idx].cur_size,
                                   wbuffer_group_[group_idx].avg_edt, min_edt,
                                   max_edt);
    }
  } else {
    bid = cur_bid;
    ...
    if (FLAGS_use_edt) {
      ...
      if (find_other_block) {
        blockid_t candidate = sm_->SelectOpenBlockID(
            GetWbufferWriteSize(wbuffer), avg_edt, min_edt, max_edt);
...
blockid_t IOInterface::SelectBlockIDToWrite(u32 idx, logid_t min_edt,
  ...
  if (!total_cnt)
    return bid;
  bid = sm_->SelectBlockIDToWrite(max_write_size, avg_edt, min_edt, max_edt);
...
std::vector<std::pair<u32, u32>> IOInterface::GroupPagesByEDT(logid_t min_edt,
                                                              logid_t max_edt) {
...
void IOInterface::BinPacking(logid_t min_edt, logid_t max_edt) {
  u64 total_cnt = 0;
  if (wbuffer_.zpage_cnt == 0)
    return;
  ...
  if (FLAGS_use_edt) {
    ...
    if (unique_edts.size() == 1) {
      ...
      std::vector<std::pair<u32, u32>> group_ranges =
          GroupPagesByEDT(min_edt, max_edt);
...
logid_t IOInterface::BinPackingPerBlock(ZipPage *src_wbuffer,
  ...
  for (int b = 0; b < (int)(bins->bincount); ++b) {
