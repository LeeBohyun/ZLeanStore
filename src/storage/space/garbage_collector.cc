...
GarbageCollector::GarbageCollector(PID2Offset *PID2Offset_table,
                                   ...
                                   u32 gc_id)
    : sspace_(storage_space), PID2Offset_table_(PID2Offset_table), fd_(fd),
      thread_id_(gc_id) {
...
bool GarbageCollector::ShouldSelectImbalancedBlock() {
...
int GarbageCollector::GreedyAmongImbalancedGroups() {
  if (!FLAGS_use_SSDWA1_pattern)
    return -1;
...
GarbageCollector::SelectVictimBlockToGCRead(int victim_cnt) {
try_again:
  ...
  if (ShouldSelectImbalancedBlock()) {
    int group_idx = GreedyAmongImbalancedGroups();
    if (group_idx != -1) {
  ...
  while ((int)victim_bids.size() < victim_cnt) {
    ...
    case 4: {
      if (FLAGS_use_SSDWA1_pattern) {
        ...
        if (victims.size() == (u64)(victim_cnt)) {
...
std::vector<blockid_t> GarbageCollector::GreedyGC(bool read, int victim_cnt) {
  ...
  while (victims.size() < (u64)(victim_cnt)) {
