...
StorageSpace::StorageSpace(u64 block_cnt, u32 page_cnt_per_block,
                           u32 min_comp_size, u64 block_size, u64 max_w_ptr,
                           u64 max_open_block_cnt)
    : blocks(block_cnt), block_metadata(block_cnt),
      min_comp_size_(min_comp_size), page_cnt_per_block_(page_cnt_per_block),
      block_cnt_(block_cnt), block_size_(block_size), max_w_ptr_(max_w_ptr),
      max_mapped_pages_cnt_(0), // optional, explicit
      max_open_block_(max_open_block_cnt) {
  if (FLAGS_use_out_of_place_write) {
    ...
    for (std::size_t i = 0; i < block_cnt; ++i) {
      ...
      block_metadata[i] = BlockMetadata(0, 0, State::EMPTY);
    ...
    fprintf(stderr, "max open block cnt: %u\n", max_open_block_);
...
SpaceManager::SpaceManager(int blockfd, u64 max_byte_offset,
                           ...
                           NVMeDevice *device)
    : block_cnt_(block_cnt), block_size_(block_alignment_size),
      max_w_ptr_(block_size), max_open_block_cnt_(max_open_block_cnt),
      cur_max_open_block_cnt_(0), // we'll set to max_open_block_cnt below
      sector_size_(sector_size), max_userspace_capacity_(max_byte_offset),
      PID2Offset_table_(nullptr), sspace_(nullptr), device_(device),
      discarded(false), max_mapped_pages_cnt_(max_mapped_pages_cnt),
      mt_offset_(0), mt_end_offset_(0), mt_fd_(blockfd) {
...
void SpaceManager::Construction() {
  ...
  if (FLAGS_use_out_of_place_write) {
    fprintf(stderr, "block cnt: %lu block_size (MB): %lu max_space (MB): %lu\n",
...
void SpaceManager::Init() {
  ...
  if (FLAGS_use_out_of_place_write) {
    ...
    sspace_ = new StorageSpace(block_cnt_, page_cnt_per_block_, min_comp_size_,
                               block_size_, max_w_ptr_, max_open_block_cnt_);
    fprintf(
        stderr,
        "block_cnt: %lu page_cnt_per_block: %u cur_max_open_block_cnt_: %u\n",
...
blockid_t SpaceManager::SelectBlockIDToWrite(u64 write_sz, logid_t avg_edt,
...
try_again:
  ...
  if (FLAGS_max_open_block_cnt) {
    bid = SelectOpenBlockID(write_sz, avg_edt, min_edt, max_edt);
    if (bid < block_cnt_) {
      return bid;
    }
    ...
    if (bid < block_cnt_) {
      ...
      if (!ShouldOpenNewBlockID(avg_edt) &&
  ...
  if (!trigger_gc) {
    bid = SelectEmptyBlockID(write_sz, avg_edt);
    if (bid < block_cnt_) {
      return bid;
    }
  }
...
blockid_t SpaceManager::SelectUnfilledOpenBlockID(logid_t avg_edt) {
...
blockid_t SpaceManager::AdjustEDTRange(logid_t global_min_edt,
                                       logid_t global_max_edt, blockid_t bid) {
  if (!FLAGS_use_edt)
    return block_cnt_;
...
bool SpaceManager::ShouldOpenNewBlockID(logid_t avg_edt) {
  std::unique_lock<std::shared_mutex> lock(sspace_->blk_state_mutex_);
  if (!FLAGS_max_open_block_cnt)
    return true;
...
blockid_t SpaceManager::SelectOpenBlockID(u64 max_write_sz, logid_t avg_edt,
  ...
  if (FLAGS_use_edt) {
    AdjustEDTRange(min_edt, max_edt, bid);
    bid = SelectOpenBlockIDWithClosestEDT(avg_edt, max_write_sz);
  } else {
    bid = SelectOpenBlockIDRandom(max_write_sz);
  }
...
blockid_t SpaceManager::SelectOpenBlockIDWithClosestEDT(logid_t avg_edt,
                                                        u64 max_write_sz) {
  blockid_t closest_bid = block_cnt_;
...
blockid_t SpaceManager::SelectFullBlockToGCRead() {
  ...
  std::vector<blockid_t> selected_bids = gc->SelectVictimBlockToGCRead(slotcnt);
...
// selection algorithm
std::vector<blockid_t> SpaceManager::SelectReadGCedBlocksToGCWrite() {
  ...
  selected_bids = gc->SelectVictimBlocksToGCWrite();
...
void SpaceManager::EraseBlock(blockid_t bid, logid_t gsn) {
...
void SpaceManager::MoveGCedBlockToOpenList(blockid_t bid, logid_t prev_min_edt,
  ...
  if (FLAGS_use_edt) {
    if (sspace_->block_metadata[bid].cur_size == 0) {
      ...
      AdjustEDTRange(global_min_edt, global_max_edt, bid);
    }
  }
...
bool SpaceManager::ShouldTriggerGCZNS() {
  if (FLAGS_max_open_block_cnt) {
