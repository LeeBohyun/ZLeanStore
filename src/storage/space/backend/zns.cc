...
ZNSBackend::ZNSBackend(int blockfd, SpaceManager *sm)
    : sm_(sm), blockfd_(blockfd), zone_nsid_(0), zone_cnt_(0), zone_size_(0),
      zone_alignment_unit_(0), max_open_zone_cnt_(0), ring_{}, request_stack(),
      submit_cnt_(0), zns_wal_cur_offset(0) {
  if (blockfd_ < 0) {
  ...
  if (FLAGS_use_ZNS) {
    ...
    if (sm_->sspace_->blocks[0].pages[0].cnt == 0 &&
      ...
      for (u32 zid = 0; zid < zone_cnt_; zid++) {
        IssueZoneReset(zid * zone_alignment_unit_);
      }
    }
...
int ZNSBackend::GetZoneNamespaceID() {
  // Fetch the namespace ID
  int nsid_ = ioctl(blockfd_, NVME_IOCTL_ID);
  if (nsid_ < 0) {
...
void ZNSBackend::ParseReportZones(u8 *buffer) {
  ...
  for (u64 i = 0; i < 2; ++i) {
    ...
    //  u8 zai = zone_desc[3];                   // Zone Attributes Info (byte
    //  3)
  ...
  fprintf(stderr, "Zone Size (MB): %lu\n", zone_size_ / (MB));
  fprintf(stderr, "Zone Alignment (MB): %lu\n", zone_alignment_unit_ / (MB));
}
...
u64 ZNSBackend::GetTotalZoneCnt() {
  ...
  cmd.opcode = 0x06;
  cmd.nsid = zone_nsid_;
  cmd.addr = reinterpret_cast<__u64>(&ns);
  cmd.data_len = ZNS_ALIGMENT;
  cmd.cdw10 = NVME_IDENTIFY_CNS_NS;
  cmd.cdw11 = NVME_CSI_NVM << 24;
  cmd.timeout_ms = 1000;
...
u32 ZNSBackend::GetMaxOpenZoneCnt() {
  ...
  cmd.nsid = zone_nsid_;
  cmd.addr = reinterpret_cast<__u64>(&zns_ns);
  cmd.data_len = ZNS_ALIGMENT;
  cmd.cdw10 = NVME_IDENTIFY_CNS_CSI_NS;
  cmd.cdw11 = NVME_NSTYPE_ZNS << 24;
  cmd.timeout_ms = 1000;
...
void ZNSBackend::IssueZoneReset(u64 zone_offset) {
  ...
  if (zone_offset == 0) {
    LOG_INFO("Zone Reset 0x%lu, zone id=%ld, successful (ioctl returned %d)",
...
void ZNSBackend::IssueZoneAppend(u64 zone_offset, void *buffer, u64 size) {
  ...
  if (zone_offset < sm_->max_userspace_capacity_) {
    IssueZoneAppendStorage(zone_offset, buffer, size);
    return;
  } else {
    IssueZoneAppendWAL(buffer, size);
  }
}

void ZNSBackend::IssueZoneAppendStorage(u64 zone_offset, void *buffer,
                                        u64 size) {
...
void ZNSBackend::IssueZoneAppendWAL(void *buffer, u64 size) {
  // wal write
  Ensure(false);
  ...
  if (leftover + size >= sm_->max_w_ptr_) {
    ...
    // First append: from zns_wal_cur_offset up to max_w_ptr_
    {
      ...
      if (offset_in_zone == 0) {
        IssueZoneReset(zns_wal_cur_offset);
      ...
      int rc = ioctl(blockfd_, NVME_IOCTL_IO_CMD, &cmd);
      if (rc < 0) {
        LOG_ERROR("Zone Append failed (first chunk), zone_id=%lu", zone_id);
        perror("ioctl failed");
        return;
      }
    ...
    // Second append: remaining part of the buffer
    {
      ...
      int rc = ioctl(blockfd_, NVME_IOCTL_IO_CMD, &cmd);
      if (rc < 0) {
        LOG_ERROR("Zone Append failed (second chunk), zone_id=%lu", zone_id);
        perror("ioctl failed");
        return;
      }
...
void ZNSBackend::ZNSRead(u64 zone_offset, void *buffer, u32 size) {
  ...
  while (size > 0) {
    ...
    u64 lba = start_lba;
    cmd.cdw10 = (u32)(lba & 0xFFFFFFFFULL);
...
void ZNSBackend::UringSubmit(size_t submit_cnt, io_uring *ring) {
  ...
  while (completed < submit_cnt) {
    ...
    static_cast<io_req_t *>(io_uring_cqe_get_data(cqe));
    if (cqe->res < 0) {
