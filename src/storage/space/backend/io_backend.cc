...
IOBackend::IOBackend(int blockfd, SpaceManager *sm,
    ...
    : blockfd_(blockfd), libaio_(blockfd), zns_(blockfd, sm),
      // ng_fd=-1, ctrl_fd=-1, ns_fd=-1, nsid=0, lba_size=4096, sm
      fdp_(-1, -1, blockfd, 0, 4096, sm), sm_(sm),
      PID2Offset_table_(PID2Offset_table), sspace_(storage_space),
      zns_wal_cur_offset_(0), trace_logger_(FLAGS_trace_file) {
  ...
  // --- FDP setup from device path ---
  if (FLAGS_use_FDP) {
    if (fdp_.InitFromDbPath(FLAGS_db_path) == 0) {
      // Use io_uring_cmd fast path if /dev/ngXnY is present; falls back to
      // ioctl otherwise (void) fdp_.InitUring(FLAGS_bm_aio_qd);

      // Optional: read FDP Config (ok to continue if it fails)
      // if (fdp_.LoadFDPConfig() != 0) {
      //   std::cerr << "[IOBackend][WARN] FDP Config read failed (errno="
      //             << errno << " " << strerror(errno) << ")\n";
      // }
    } else {
      std::cerr << "[IOBackend][WARN] FDP InitFromDbPath failed: "
                << strerror(errno) << "\n";
    }
  }
...
void IOBackend::WriteWAL(u64 offset, u32 write_sz, void *buffer,
  ...
  if (FLAGS_use_ZNS) {
    zns_.IssueZoneAppend(offset, buffer, write_sz);
    return;
  }

  if (FLAGS_use_FDP) {
    u32 rc_ru = fdp_.ru_cnt() ? fdp_.ru_cnt() : 8; // fallback
    const u16 plid = static_cast<u16>(
        (sm_->block_size_ ? (offset / sm_->block_size_) : 0) % rc_ru);
    fdp_.WriteWithPLID(buffer, write_sz, offset / fdp_.lba_size(), plid);
    fdp_.UringFlushWrites();
    return;
  }
}
...
void IOBackend::WritePagesOutOfPlace(u64 offset, u32 write_sz, void *buffer,
  ...
  if (FLAGS_use_FDP) {
    // u32 rc_ru = fdp_.ru_cnt() ? fdp_.ru_cnt() : 8; // fallback
    const u16 plid = static_cast<u16>(
        (sm_->block_size_ ? (offset / sm_->block_size_) : 0) % fdp_.ru_cnt());
    fdp_.WriteWithPLID(buffer, write_sz, offset / fdp_.lba_size(), plid);
    fdp_.UringFlushWrites();
    return;
  }
...
void IOBackend::ReadPagesOutOfPlace(std::vector<u64> &offsets,
                                    std::vector<void *> &buffers,
                                    bool synchronous) {
  ...
  if (!FLAGS_use_ZNS) {
    // use io_uring for also FDP enabled
    for (u32 idx = 0; idx < offsets.size(); idx++) {
  ...
  if (FLAGS_use_ZNS) {
    for (u32 idx = 0; idx < offsets.size(); idx++) {
      zns_.ZNSRead(offsets[idx], buffers[idx], FLAGS_read_size);
    }
    return;
  }
}
...
void IOBackend::ReadPage(u64 offset, void *buffer, u16 read_size, bool sync) {
  ...
  if (FLAGS_use_ZNS) {
    zns_.ZNSRead(offset, buffer, read_size);
    return;
  }
}
