...
NVMeDevice::NVMeDevice() {
  ...
  GetMaxLogicalPID();
  LOG_INFO(
      "max page cnt: %lu zone_cnt: %lu db_zone_cnt: %lu max_byte_offset_ %lu max_db_byte_offset: %lu",
      max_mapped_pid_cnt_, zone_cnt_, db_zone_cnt_, max_byte_offset_, max_db_byte_offset_);
...
void NVMeDevice::WriteToSSD(u64 offset) {
  if (FLAGS_simulator_mode) {
    ...
    ensure(pid < max_byte_offset_/ PAGE_SIZE);
...
int NVMeDevice::nvmeGetNamespaceID(u32 nsid, enum nvme_identify_cns cns,
  ...
  struct nvme_passthru_cmd cmd = {}; // zero-initialize all fields

  cmd.opcode = nvme_admin_identify;
  cmd.nsid = nsid;
  cmd.addr = (u64)(uintptr_t)data;
  cmd.data_len = NVME_IDENTIFY_DATA_SIZE;
  cmd.cdw10 = cns;
  cmd.cdw11 = (u32)csi << 24;
...
void NVMeDevice::nvmeGetInfo() {
  ...
  int nsid = ioctl(blockfd_, NVME_IOCTL_ID);
  if (nsid < 0) {
    fprintf(stderr, "Error: failed to fetch namespace ID\n");
    close(blockfd_);
    return;
  }
  nsid_ = static_cast<u32>(nsid);
  ...
  if (FLAGS_use_ZNS) {
    ...
    if (FLAGS_use_out_of_place_write) {
      ...
      max_open_zone_cnt_ =
          FLAGS_max_open_block_cnt > 0 ? FLAGS_max_open_block_cnt : zone_cnt_;
      fprintf(stderr, "max open zone cnt: %u, zone_cnt: %lu\n",
...
u64 NVMeDevice::nvmeGetCap() {
  ...
  cmd.opcode = nvme_admin_identify; // NVMe Identify command
  cmd.nsid = nsid_;
  cmd.addr = (u64)(uintptr_t)&ns; // make sure this matches the type of
                                  // nvme_passthru_cmd::addr
  cmd.data_len = 4096;            // or sizeof(ns)
  cmd.cdw10 = NVME_IDENTIFY_CNS_NS;
  cmd.cdw11 = (u32)NVME_CSI_NVM << 24;
  cmd.timeout_ms = 1000;
  ...
  fprintf(
      stderr,
      "Namespace Capacity (ncap): %lu blocks, Logical Block Size: %u bytes\n",
      ncap, sector_size_);
...
u64 NVMeDevice::nvmeGetInfoZNS() {
  ...
  // NVMe Identify Namespace command
  struct nvme_passthru_cmd cmd {}; // zero-initialize all fields safely

  cmd.opcode = nvme_admin_identify; // NVMe Identify command
  cmd.nsid = nsid_;
  cmd.addr = (u64)(uintptr_t)&ns; // correct cast for 64-bit address
  cmd.data_len = 4096;            // namespace data structure size
  cmd.cdw10 = NVME_IDENTIFY_CNS_NS;
  cmd.cdw11 = (u32)NVME_CSI_NVM << 24;
  cmd.timeout_ms = 1000;
...
u64 NVMeDevice::ZNSGetAlignmentSize() {
  ...
  for (u64 i = 0; i < 2; ++i) {
    ...
    std::cout << "Zone " << i << " => " << " SLBA: " << std::dec << slba_decimal
              << " WP: " << wp << " Cap: " << cap_decimal
...
u64 NVMeDevice::ZNSGetTotalZoneCnt() {
  ...
  // NVMe Identify Namespace command
  struct nvme_passthru_cmd cmd {}; // zero-initialize everything

  cmd.opcode = 0x06; // NVMe Identify
  cmd.nsid = nsid_;
  cmd.addr = (u64)(uintptr_t)&ns; // ensure 64-bit address
  cmd.data_len = sector_size_;    // size of namespace info struct
  cmd.cdw10 = NVME_IDENTIFY_CNS_NS;
  cmd.cdw11 = (u32)NVME_CSI_NVM << 24;
  cmd.timeout_ms = 1000;
...
u32 NVMeDevice::ZNSGetMaxOpenZoneCnt() {
  ...
  cmd.opcode = 0x06;
  cmd.nsid = nsid_;
  cmd.addr = (u64)(uintptr_t)&zns_ns;
  cmd.data_len = sector_size_;
  cmd.cdw10 = NVME_IDENTIFY_CNS_CSI_NS;
  cmd.cdw11 = (u32)NVME_NSTYPE_ZNS << 24;
  cmd.timeout_ms = 1000;
  ...
  if (mor == 0) {
    ...
    LOG_INFO("Maximum Open Zones Count (MOR): %u", mor); // Log the MOR value
  }
...
void NVMeDevice::nvmeGetWALInfo() {
  ...
  if (FLAGS_db_path == FLAGS_wal_path) {
    ...
    if (FLAGS_use_ZNS) {
      ...
      fprintf(stderr, "required zone cnt for WAL log: %u\n", required_zone_cnt);
    ...
    if (FLAGS_use_ZNS) {
      u32 required_zone_cnt =
          (FLAGS_max_wal_capacity_gb * GB + zone_alignment_size_ - 1) /
          zone_alignment_size_;
      fprintf(stderr, "required_zone_cnt: %u zone_alignment_size: %lu\n",
  ...
  if (!FLAGS_use_out_of_place_write) {
    ...
    if (!FLAGS_use_compression) {
      ...
      fprintf(stderr,
              ...
              "db_zone_cnt: %lu max_pid: %lu\n",
              max_db_byte_offset_ / GB, max_db_size / GB, zone_cnt_,
              db_zone_cnt_, max_mapped_pid_cnt_);
    } else {
      // enable compression
      // maximum 4 PIDs per 4096 bytes (since MIN_COMP_SIZE == 2048)
      if (max_db_byte_offset_ < max_db_size) {
        max_db_size = max_db_byte_offset_;
      }
      ...
      fprintf(
          stderr,
          "max db byte offset (GB): %lu max_db_size (GB): %lu zone_cnt: %lu "
          "db_zone_cnt: %lu max_pid: %lu\n",
          max_db_byte_offset_ / GB, max_db_size / GB, zone_cnt_, db_zone_cnt_,
          max_mapped_pid_cnt_);
    }
  }
  return;
