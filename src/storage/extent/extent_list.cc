...
auto TailExtent::SplitToExtents(
    pageid_t start_pid, u64 page_cnt,
    const std::function<void(pageid_t, extidx_t)> &cb) -> u32 {
  auto index = ExtentList::NO_TIERS - 1;
  u32 count = 0;

  while (page_cnt > 0) {
    while (page_cnt < ExtentList::TIER_SIZE[index]) {
      --index;
    }
    cb(start_pid, index);
    count++;
...
auto TailExtent::SplitToExtents(
    const std::function<void(pageid_t, extidx_t)> &cb) const -> u32 {
