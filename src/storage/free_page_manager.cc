...
inline void FreePageManager::LogFreePage(bool is_free_op, pageid_t start_pid,
                                         extidx_t tier_idx) {
  auto &txn = transaction::TransactionManager::active_txn;
  Ensure(txn.IsRunning());
  if (!is_free_op) {
    statistics::storage::free_size -= ExtentList::TIER_SIZE[tier_idx];
  }
  txn.LogWorker().ReserveFreePageLogEntry(is_free_op, start_pid,
                                          ExtentList::ExtentSize(tier_idx));
}

/**
 * @brief Find a free page that is at least `required_page_cnt` pages which
 * satisfies `validate_extent` If there is no suitable range -> return `false`
 *
 * @param required_page_cnt   Minimum number of required page count
 * @param validate_extent     Validation function to check if the free range is
 * suitable
 * @param out_pid             *Output* Start PID of the free extent
 * @param out_split_exts      *Output* Information about all the splitted
 * smaller extents
 */
auto FreePageManager::RequestFreeExtent(
    u64 required_page_cnt, const std::function<bool(pageid_t)> &validate_extent,
    ...
    if (tiers_[tier_index].try_pop(out_pid)) {
      if (validate_extent(out_pid)) {
        ...
        if (ExtentList::TIER_SIZE[tier_index] > required_page_cnt) {
          PrepareFreePages(out_pid + required_page_cnt,
                           ExtentList::TIER_SIZE[tier_index] -
                               required_page_cnt,
                           out_split_exts);
        }
        return true;
  ...
  /**
   * @brief Consider the top tier at the end, because the number of biggest
   * extents should be as high as possible
   */
  for (auto i = ExtentList::NO_TIERS - 2; i >= tier_index; i--) {
    ...
    if (tiers_[x_tier].try_pop(out_pid)) {
      if (validate_extent(out_pid)) {
        ...
        PrepareFreePages(out_pid + required_page_cnt,
                         ExtentList::TIER_SIZE[x_tier] - required_page_cnt,
                         out_split_exts);
        return true;
      }
 ...
 * If the transaction aborts, then the vector will be clear, i.e. no free range
...
/**
 * @brief Free a large page, starting at `start_pid` and lasting for `page_cnt`
 * pages into multiple extents which fit random tiers
 */
void FreePageManager::PrepareFreePages(pageid_t start_pid, u64 page_cnt,
                                       TierList &out_split_exts) {
  storage::TailExtent::SplitToExtents(
      start_pid, page_cnt, [&](pageid_t start_pid, extidx_t index) {
