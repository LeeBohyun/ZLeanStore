...
WorkerPool::WorkerPool(std::atomic<bool> &keep_running)
  ...
  for (wid_t t_i = 0; t_i < FLAGS_worker_count; t_i++) {
    workers_.emplace_back([&, t_i]() {
      ...
      while (is_running_->load()) {
        ...
        meta.cv.wait(guard,
                     [&]() { return !is_running_->load() || meta.job_set; });
        if (!is_running_->load()) {
          break;
        }
        meta.wt_ready = false;
  ...
  while (threads_counter_ < FLAGS_worker_count) {
    AsmYield();
  }
}
...
void WorkerPool::Stop() {
  if (is_running_->load()) {
    ...
    for (wid_t t_i = 0; t_i < WorkersCount(); t_i++) {
      worker_env_[t_i].cv.notify_one();
    }
    JoinAll();
    while (threads_counter_ > 0) {
      AsmYield();
    }
  }
}
...
// -------------------------------------------------------------------------------------
void WorkerPool::ScheduleAsyncJob(wid_t wid, const std::function<void()> &job) {
  SetJob(wid, job);
}
...
void WorkerPool::JoinAll() {
  // LOG_INFO("worker_count: %d", WorkersCount());
  for (wid_t t_i = 0; t_i < WorkersCount(); t_i++) {
    // LOG_INFO("joinworker t_i: %d", t_i);
    JoinWorker(t_i, [&](WorkerThreadEnv &meta) {
      return meta.wt_ready && !meta.job_set;
    });
  }
}
...
void WorkerPool::JoinWorker(wid_t wid,
                            std::function<bool(WorkerThreadEnv &)> condition) {
  Ensure(wid < WorkersCount());
  auto &meta = worker_env_[wid];
  std::unique_lock guard(meta.mutex);
