...
public:
  ...
  void WritePages(std::vector<pageid_t> &to_write, bool evict);
  ...
  // for GC operations
  u64 WriteDirtyPages(u32 write_cnt, u32 shard_idx, u32 shard_cnt);
  void WritePagesInPlace(std::vector<pageid_t> &to_write, bool evict);
  void WritePagesOutOfPlace(bool evict);
  void HandlePageFaultGC(pageid_t page_id);
  blockid_t GCValidPIDs(blockid_t bid, u32 group_idx);
  std::vector<pageid_t> GCReadValidPIDs(blockid_t bid);
