...
public:
  ...
  static constexpr u64 MAX_SHARED =
      254; // # share-lock holders (1 -> MAX_SHARED)
 ...
 *   - EXCLUSIVE
 *
 * Additional states should be placed after EXCLUSIVE, e.g.
 * buffer::PageStateMode The size of those states should be not bigger than 8
 * bytes
 */
template <class SyncStateClass> class HybridLatchImpl : public SyncStateClass {
public:
...
protected:
  ...
  // Assertion utilities
  auto IsExclusivelyLatched(u64 v) -> bool {
    return LockState(v) == SyncStateClass::EXCLUSIVE;
  }

  auto IsSharedLatched(u64 v) -> bool {
    auto curr = LockState(v);
    return (SyncStateClass::UNLOCKED < curr) &&
           (curr < SyncStateClass::EXCLUSIVE);
  }

  // State & Version utilities
  static auto SameVersionNewState(u64 old_state_and_version, u64 new_state)
      -> u64 {
    return static_cast<u64>(((old_state_and_version << 8) >> 8) | new_state
                                                                      << 56);
  }

  static auto NextVersionNewState(u64 old_state_and_version, u64 new_state)
      -> u64 {
    return static_cast<u64>((((old_state_and_version << 8) >> 8) + 1) |
                            new_state << 56);
  }
