...
template <class PageClass> class SharedGuard;
template <class PageClass> class ExclusiveGuard;

template <class PageClass> class PageGuard {
public:
  PageGuard();
  PageGuard(pageid_t pid, PageClass *ptr, PageState *page_state,
            GuardMode mode);
  ...
  auto ReserveWalEntry(u64 wal_payload_size) -> WalType & {
    // We have too many WAL types, hence impl this func here helps reduce the
    // code size
    ...
    auto &entry = static_cast<WalType &>(logger.ReserveDataLog(
        sizeof(WalType) + wal_payload_size, page_id_, page_ptr_->idx_id));
    // Update its WAL type
    ...
    } else if (std::is_same_v<WalType, storage::WALUpdate>) {
      entry.type = storage::WalType::UPDATE;
    } else {
      UnreachableCode();
    }
    return entry;
