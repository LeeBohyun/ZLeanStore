...
/* Caller of SharedGuard should S-lock the PageState before constructing the
 * Guard */
template <class PageClass> class SharedGuard : public PageGuard<PageClass> {
public:
  SharedGuard() : PageGuard<PageClass>::PageGuard() {}

  SharedGuard(pageid_t pid, PageClass *ptr, PageState *page_state);
