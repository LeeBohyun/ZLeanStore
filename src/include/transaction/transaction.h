...
class Transaction {
public:
  enum class Type { USER, SYSTEM, VOID };
...
#endif
  void Initialize(TransactionManager *manager, timestamp_t start_ts,
  ...
  template <class PageClass> void AdvancePageGSN(PageClass *page, pageid_t pid);
...
private:
  ...
  logid_t max_observed_gsn_; // The smallest upper-limit of gsn at which all
                             // logs of all workers have been flushed
  bool needs_remote_flush_;  // whether active_txn can avoid remote flush before
                             // commit
