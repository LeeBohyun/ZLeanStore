...
template <int TypeID> struct BlobRelation {
  ...
  auto PayloadSize() const -> uint32_t {
    return reinterpret_cast<const storage::blob::BlobState *>(payload)
        ->MallocSize();
  }
 ...
 */
static constexpr uint32_t MAX_INROW_SIZE =
    storage::BTreeNode::MAX_RECORD_SIZE - sizeof(uint16_t) - sizeof(uint64_t);

template <int TypeID> struct InrowBlobRelation {
  static constexpr int TYPE_ID = TypeID;
  ...
  InrowBlobRelation(uint64_t inrow_size, std::span<const uint8_t> s)
      : inrow_size(inrow_size) {
    if (inrow_size > 0) {
      Ensure(inrow_size == s.size());
    }
  ...
  auto PayloadSize() const -> uint32_t {
    ...
    if (inrow_size == 0) {
      ret += reinterpret_cast<const storage::blob::BlobState *>(payload)
                 ->MallocSize();
    } else {
      ret += inrow_size;
    }
