...
**      |---------------------|
**
**  The payload area of each record contains both key + value (i.e. payload) of
*the record
**  While the PageSlot only maintain the sorted order & the offset to the record
**
**  The relationship between parent's separators and children are depicted as
*below
**  Note that both children are leaf nodes.
**  For inner nodes, both left's keys vs X vs right's keys are exclusive, i.e.
*left's keys < X < right's keys
**
**                    |------------------|
**                    |      parent      |
**                    | ptr |  X   | ptr |
...
struct SeparatorInfo {
  ...
  bool is_truncated; // if true, we truncate the separator taking len bytes from
                     // slot+1
};
...
public:
  static constexpr u64 SIZE_UNDER_FULL =
      PAGE_SIZE * 3 / 4; // merge nodes below this size
  ...
  union {
    ...
    pageid_t next_leaf_node =
        EMPTY_NEIGHBOR; // for leaf nodes: pageid_t of the right sibling
  };
  // pageid_t next_leaf_node =
  //       EMPTY_NEIGHBOR; // for leaf nodes: pageid_t of the right sibling
  pageid_t prev_leaf_node =
      EMPTY_NEIGHBOR; // for leaf nodes: pageid_t of the left sibling
  ...
  leng_t data_offset =
      static_cast<leng_t>(PAGE_SIZE); // where we start moving payload to
  leng_t prefix_len = 0;
...
public:
  static constexpr u32 MAX_RECORD_SIZE =
      ((PAGE_SIZE - sizeof(BTreeNodeHeader) - (3 * sizeof(PageSlot)))) / 3;
  ...
  auto FindChild(std::span<u8> search_key, const ComparisonLambda &cmp)
      -> pageid_t;
  auto FindChild(std::span<u8> search_key, leng_t &pos,
  ...
  void SearchHint(const std::array<u8, 4> &key_head, leng_t &out_lower_bound,
                  leng_t &out_upper_bound);

  // Search utilities
  auto LowerBound(std::span<u8> search_key, bool &exact_found,
                  const ComparisonLambda &cmp) -> leng_t;
  auto LowerBound(std::span<u8> search_key, const ComparisonLambda &cmp)
      -> leng_t;

  // Store/Insert/Remove utilities
  void StoreRecordData(leng_t slot_id, std::span<u8> key,
                       std::span<const u8> payload);
  void StoreRecordDataWithoutPrefix(leng_t slot_id, std::span<u8> key,
                                    std::span<const u8> payload);
  void InsertKeyValue(std::span<u8> key, std::span<const u8> payload,
                      const ComparisonLambda &cmp);
  ...
  void CopyKeyValueRange(BTreeNode *dst, leng_t dst_slot_start,
                         leng_t src_slot_start, leng_t src_count);

  // Split/Merge utilities
  auto FindSeparator(bool append_bias, const ComparisonLambda &cmp)
      -> SeparatorInfo;
  void SplitNode(BTreeNode *parent, BTreeNode *node_right, pageid_t left_pid,
                 pageid_t right_pid, leng_t separator_slot,
                 std::span<u8> sep_key, const ComparisonLambda &cmp);
  auto MergeNodes(leng_t left_slot_id, BTreeNode *parent, BTreeNode *right,
                  const ComparisonLambda &cmp) -> bool;
  auto MergeNodesFromRight(leng_t right_slot_id, BTreeNode *parent,
                           BTreeNode *right, const ComparisonLambda &cmp)
      -> bool;

  // BLOB custom LookUp operators
  auto FindChildWithBlobKey(const blob::BlobLookupKey &key, leng_t &pos,
                            const ComparisonLambda &cmp) -> pageid_t;
  auto LowerBoundWithBlobKey(const blob::BlobLookupKey &key, bool &exact_found,
                             const ComparisonLambda &cmp) -> leng_t;
