...
#define WalNewTuple(node_guard, wal_type, key, w_payload)                      \
  {                                                                            \
    auto &entry =                                                              \
        (node_guard)                                                           \
            .ReserveWalEntry<wal_type>((key).size() + (w_payload).size());     \
    std::tie(entry.key_length, entry.value_length) =                           \
        std::make_tuple((key).size(), (w_payload).size());                     \
    std::memcpy(entry.payload, (key).data(), (key).size());                    \
    std::memcpy(entry.payload + (key).size(), (w_payload).data(),              \
                (w_payload).size());                                           \
