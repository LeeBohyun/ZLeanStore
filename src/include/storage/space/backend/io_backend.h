...
public:
  ...
  void LogWrite(u64 offset, pageid_t pid, u64 idx_id) {
    Log("W," + std::to_string(pid) + "," + std::to_string(idx_id) + "," +
        std::to_string(offset) + "\n");
  }
...
public:
  ...
  void ReadPagesOutOfPlace(std::vector<u64> &offsets,
                           std::vector<void *> &buffers, bool synchronous);

private:
  int blockfd_;
  storage::space::backend::LibaioInterface libaio_;
  storage::space::backend::ZNSBackend zns_;
  storage::space::backend::FDPBackend fdp_;
