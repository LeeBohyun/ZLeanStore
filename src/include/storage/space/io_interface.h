...
public:
  ...
  u16 CompressPage(Page *page, char *wbuffer, u64 start_ptr);
  void UpdateCachedPageInfo(ZipPage *wbuffer, pageid_t pid, u16 comp_sz,
                            logid_t edt);
  void BinPacking(logid_t min_edt, logid_t max_edt);
  void SortPagesByEDT();
  std::vector<logid_t> ExtractUniqueEDTs();
  std::vector<std::pair<u32, u32>> GroupPagesByEDT(logid_t min_edt,
                                                   logid_t max_edt);
  ...
  void ResetWBufferPerGroup(u32 page_cnt);
