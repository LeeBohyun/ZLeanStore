...
struct PID2Offset {
  u64 packed_data __attribute__((packed));
...
struct StorageSpace {
  ...
  u64 max_mapped_pages_cnt_ =
      0; // == max_db_capacity_gb * GB /PAGE_SIZE if compression disabled
  ...
  StorageSpace(u64 block_cnt, u32 page_cnt_per_block, u32 min_comp_size,
  ...
  u32 GetActiveBlockSize() {
    ...
    auto &open_list = block_lists[State::OPEN];
    cnt = open_list.size();
    return cnt;
  }
};
...
public:
  ...
  blockid_t SelectBlockIDToWrite(u64 max_write_sz, logid_t avg_wgap,
  ...
  blockid_t SelectOpenBlockIDRandom(u64 max_write_sz);
  ...
  std::vector<blockid_t> SelectReadGCedBlocksToGCWrite();
  blockid_t SelectUnfilledOpenBlockID(logid_t avg_edt);
  blockid_t SelectGCInUseBlockToGCRead();
  bool ShouldOpenNewBlockID(logid_t avg_edt);
  ...
  void EraseBlock(blockid_t bid, logid_t gsn);
