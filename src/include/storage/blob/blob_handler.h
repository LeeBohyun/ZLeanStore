 ...
 * A single Blob can only be referred to by exactly one BlobHandler,
 *  and all Blobs whose have the same root_id are considered to be the same Blob
 *
 * Theoretically, as long as a Page can contain at least two Blob Handlers, then
 * Index will work In other words, a Blob Handler could be approx as big as
 * (PAGE_SIZE - PAGE_HEADER_SIZE) / 2 We can safely assume a MAX_MALLOC_SIZE of
 * 1700 bytes, which corresp. to (1700 - MIN_MALLOC_SIZE) / 8 = 202 extents
 * Using the Power-of-Two formula, i.e. when ExtentList::EXTENT_PER_LEVEL = 202
 * and ExtentList::NUMBER_OF_LEVELS = 1 The maximum BLOB size is 2.18 * 10^40 YB
 *
 * Here, we want set a limit of 127 extents (7 bits, 1 reserved bit for later
 * use) per Blob Handler. Therefore, the maximum malloc size is 1100 bytes.
 * Advantages:
 * - Beautiful number. Who doesn't like perfection?
 * - Each page can store at least 3 Blob Handlers, i.e. more efficient indexing
...
struct BlobHandler {
  ...
  // -------------------------------------------------------------------------------------
  auto operator!=(const BlobHandler &other) -> bool {
  ...
  // -------------------------------------------------------------------------------------
  /**
   * @brief Size of a BlobHandler with specified number of extents
   */
  static constexpr auto MallocSize(uint8_t no_extents) -> uint16_t {
    return sizeof(BlobHandler) + sizeof(pageid_t) * no_extents;
  }
  ...
  static auto MoveToTempStorage(u8 *tmp,
                                BlobHandler *tmp_btup) -> BlobHandler * {
    std::memcpy(tmp, tmp_btup, tmp_btup->MallocSize());
    return reinterpret_cast<BlobHandler *>(tmp);
  }
...
struct BlobLookupKey {
  ...
  uint8_t sha2_digest[BlobHandler::SHA256_DIGEST_LENGTH] = {
      0}; // SHA-2 value, used for hash operators

  explicit BlobLookupKey(std::span<const u8> blob_data) : blob(blob_data) {
    if (blob.size() > BlobHandler::PREFIX_LENGTH) {
      BlobHandler::CalculateSHA256(sha2_digest, blob);
    }
  }
};

// Initial size of BlobHandler should be fixed
static_assert(sizeof(BlobHandler) == BlobHandler::MIN_MALLOC_SIZE);
static_assert(BlobHandler::MallocSize(0) == BlobHandler::MIN_MALLOC_SIZE);
static_assert(BlobHandler::MallocSize(ExtentList::EXTENT_CNT_MASK) ==
              BlobHandler::MAX_MALLOC_SIZE);
