...
TransactionManager::TransactionManager(buffer::BufferManager *buffer_manager,
                                       LogManager *log_manager)
    : buffer_(buffer_manager), log_manager_(log_manager){};

auto TransactionManager::ParseIsolationLevel(const std::string &str)
    -> IsolationLevel {
  if (str == "ser") {
...
void TransactionManager::StartTransaction(
    Transaction::Type next_tx_type, IsolationLevel next_tx_isolation_level,
    Transaction::Mode next_tx_mode, bool read_only) {
  Ensure(!active_txn.IsRunning());
  active_txn.Initialize(this, global_clock++, next_tx_type,
                        next_tx_isolation_level, next_tx_mode, read_only);
  if (FLAGS_txn_debug) {
  ...
  if (next_tx_isolation_level >= IsolationLevel::READ_COMMITTED) {
    throw leanstore::ex::TODO("Not implemented yet");
  }
}

void TransactionManager::CommitTransaction() {
  // If WAL is disabled or current transaction is read-only, commit
  // transaction'll be a no-op
  ...
  if (FLAGS_wal_enable && !active_txn.ReadOnly()) {
    ...
    // Lock the mutex to safely access the queues
    {
      std::unique_lock<std::mutex> lock(logger.precommitted_queue_mutex);

      // Copy the active transaction before pushing it to the queue
      Transaction txn_copy =
          active_txn; // Create a copy of the active transaction

      if (txn_copy.needs_remote_flush_) {
        logger.precommitted_queue.push_back(txn_copy);
        if (start_profiling && worker_thread_id < FLAGS_worker_count) {
          statistics::precommited_txn_processed[worker_thread_id] += 1;
        }
      } else {
        logger.precommitted_queue_rfa.push_back(txn_copy);
        if (start_profiling && worker_thread_id < FLAGS_worker_count) {
...
void TransactionManager::AbortTransaction() {
  ...
  if (active_txn.ReadOnly()) {
    return;
  }
  if (FLAGS_txn_debug) {
    Ensure(!active_txn.wal_larger_than_buffer);
  }
  ...
  logger.IterateActiveTxnEntries([&](const LogEntry &entry) {
    if (entry.type == LogEntry::Type::DATA_ENTRY) {
      entries.push_back(&entry);
    }
  });
  std::for_each(entries.rbegin(), entries.rend(), [&](const LogEntry *entry) {
    [[maybe_unused]] const auto &data_entry =
        *reinterpret_cast<const DataEntry *>(entry); // NOLINT
