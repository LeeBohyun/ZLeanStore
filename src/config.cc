...
DEFINE_string(
    db_path, "/dev/nvme2n1",
    "Default database path, can be a file directory or a raw block device");
DEFINE_string(db_dir, "", "Default database path, if filesystems are used");
DEFINE_uint64(max_ssd_capacity_gb, 64, "Maximum SSD fill capacity");
DEFINE_string(
    wal_path, "/dev/nvme2n1",
...
DEFINE_bool(use_out_of_place_write, false,
            "true: store pages out-of-place" // gc enabled auto matically
            "false: store pages in place");
/* Block (i.e., GC unit) size */
DEFINE_uint32(block_size_mb, 64, "Size of a single block");
...
/* Garbage collection */
DEFINE_uint32(
    garbage_collector_cnt, 0, // used when allowing background gc, otherwise it will all run foreground
...
DEFINE_uint32(
    ...
    "4: 2R-Greedy"); // if nowa is enabled, then NoWA is automatically considered
/* Use compression */
...
DEFINE_uint32(
    bm_aio_qd, 64,
    "Maximum number of concurrent I/O processed by backend's io_uring, per thread");
// -----------------------------------------------------------------------------------
/* SSD interface */
DEFINE_string(user_pwd, "", "User pwd for OCP command and trim command");
DEFINE_bool(use_trim, false, "use trim command");
DEFINE_bool(measure_waf, true,
            "true: measure SSD write amplification based on OCP results");
...
DEFINE_bool(use_SSDWA1_pattern, false, "true: use SSD1WA 1 magic NoWA pattern");
DEFINE_bool(batch_writes, false, "batch writes into a single writes");
// -----------------------------------------------------------------------------------
/* log page traces */
DEFINE_string(trace_file, "",
              "IO trace file directory, empty means trace logging is disabled");
/* SSD WAF simulator, to estimate SSD WAF */
DEFINE_bool(simulator_mode, false, "SSD simulator mode, gives you the simulated SSD WAF ");
DEFINE_string(simulator_SSD_gc, "greedy", "GC algorithm of SSD simulator");
DEFINE_uint32(simulator_SSD_gc_unit_mb, 64, "GC granularity in SSD simulator");
DEFINE_double(SSD_OP, 0.125,
              "0.1: assuming the simulator SSD has 12.5pct of the "
              "overprovisioning space internally");