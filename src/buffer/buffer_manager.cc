...
BufferManager::BufferManager(int blockfd, u64 virtual_page_count,
    ...
    : sm_(sm), blockfd_(blockfd),
      min_clean_cnt_((FLAGS_block_size_mb * MB / PAGE_SIZE) *
                     (FLAGS_worker_count + FLAGS_page_provider_thread +
                      FLAGS_wal_enable + FLAGS_checkpointer_cnt +
                      FLAGS_garbage_collector_cnt)),
      is_loading_(&is_loading), virtual_size_(virtual_page_count * PAGE_SIZE),
      ...
      io_handler_cnt_(FLAGS_worker_count + FLAGS_page_provider_thread +
                      FLAGS_wal_enable + FLAGS_checkpointer_cnt + FLAGS_wal_enable +
                      FLAGS_garbage_collector_cnt) {
  Construction();
}
...
BufferManager::BufferManager(int blockfd, u64 virtual_page_count,
    ...
    : sm_(sm), blockfd_(blockfd),
      ...
      alias_pg_cnt_(extra_page_count), evict_batch_(evict_size),
      keep_running_(&keep_running), free_pages_(fp),
...
pageid_t BufferManager::ReuseDeallocatedSpace() {
  ...
  if (sm_->HasReusablePID(worker_thread_id)) {
    ...
    Ensure(frame_[space_pid].deallocated);
    sm_->UpdatePID2Offset(space_pid, 0, 0, sm_->block_cnt_, false);
...
void BufferManager::HandlePageFaultGC(pageid_t pid) {
  ...
  if (physical_used_cnt_ >= physical_cnt_ - 1) {
    ...
    LOG_INFO("physical used: %lu physical_cnt: %lu",
         physical_used_cnt_.load(std::memory_order_relaxed),
         physical_cnt_);
  ...
  sm_->UpdatePID2Offset(pid, 0, 0, 0, true);
...
bool BufferManager::EvictDeallocPage(pageid_t pid) {
  ...
  if (FLAGS_use_out_of_place_write) {
    sm_->UpdatePID2Offset(pid, 0, PAGE_SIZE - 1, 0, false);
  } else {
    sm_->UpdatePID2Offset(pid, 0, PAGE_SIZE - 1, sm_->block_cnt_, false);
...
void BufferManager::WritePages(std::vector<pageid_t> &to_write, bool evict) {
  if (to_write.size() == 0) {
    return;
  }
  ...
  for (auto &pid : to_write) {
    ...
    if (FLAGS_use_edt) {
      ...
      edt = ToPtr(pid)->EstimateDeathTime();
    }
...
void BufferManager::WritePagesOutOfPlace(bool evict) {
  // oop write

  // binpacking into groups
  io_interface_[worker_thread_id].BinPacking(min_edt.load(), max_edt.load());
...
u64 BufferManager::GCValidPIDs(blockid_t read_bid, u32 group_idx) {
  // write previously gc-read blocks
  GCWriteValidPIDs(group_idx);

  // get the number of pids on disk to read
  GCValidPIDCntInBlockOnDisk(read_bid);

  // read valid pids from the disk
  std::vector<pageid_t> read_pids = GCReadValidPIDs(read_bid);
...
blockid_t BufferManager::GCWriteValidPIDs(u32 idx) {
  ...
  io_interface_[worker_thread_id].ResetGCWriteBuffer(gc_batch_);
  ...
  io_interface_[worker_thread_id].ResetGCWriteBuffer(0);
...
void BufferManager::UpdateMinMaxEDT(pageid_t pid) {
  if (!FLAGS_use_edt)
    return;
  logid_t edt = ToPtr(pid)->EstimateDeathTime();
...
void BufferManager::RemoveMinMaxEDT(pageid_t pid) {
  ...
  logid_t target_edt = ToPtr(pid)->EstimateDeathTime();

  if (ToPtr(pid)->gc_read &&
      ToPtr(pid)->prev_gcwrite <
          recovery::LogManager::global_min_gsn_flushed.load()) {
    target_edt =
        ToPtr(pid)->EstimateDeathTime() >=
                recovery::LogManager::global_min_gsn_flushed.load()
            ? ToPtr(pid)->EstimateDeathTime()
...
BufferManager::GCReadValidPIDsInBlockOnDisk(blockid_t bid) {
  ...
  for (u32 p = 0; p < sspace_->page_cnt_per_block_; p++) {
    ...
    for (u8 pidx = 0; pidx < pcnt; pidx++) {
      ...
      if (physical_used_cnt_ >= physical_cnt_ - evict_batch_) {
        break;
      }
  ...
  if (!to_read.empty()) {
    ...
    EvictCleanPages(to_read.size(), bid);
...
u64 BufferManager::WriteDirtyPages(u32 write_cnt, u32 shard_idx,
                                   u32 shard_cnt) {
  if (write_cnt == 0) {
    return 0;
  }
...
logid_t BufferManager::FuzzyCheckpoint(logid_t target_gsn, u32 shard_idx,
  ...
  // if (FLAGS_use_out_of_place_write &&
  //     clean_page_cnt < sspace_->page_cnt_per_block_ / shard_cnt) {
  //   u64 write_cnt = target_gsn == 1 ? sspace_->page_cnt_per_block_ / shard_cnt
  //                                   : sspace_->page_cnt_per_block_ / shard_cnt;

  //   // write_cnt = RoundUpToPowerOfTwo(write_cnt);
  //   write_cnt = evict_batch_;
  //   u64 cur_w_cnt = 0;
  //   while (cur_w_cnt < write_cnt) {
  //     // u64 target_cnt = write_cnt - cur_w_cnt >= evict_batch_
  //     //                      ? evict_batch_
  //     //                      : write_cnt - cur_w_cnt;
  //     u64 cur_cnt = WriteDirtyPages(evict_batch_, shard_idx, shard_cnt);
  //     cur_w_cnt += cur_cnt;
...
#ifdef ENABLE_EXMAP
      if (FLAGS_use_exmap)
        ExmapAlloc(out_start_pid, required_page_cnt);
#endif
      if (is_special_block) {
 ...
 * TailExtent
 */
auto BufferManager::AllocExtent(extidx_t extent_idx,
                                u64 fixed_page_cnt) -> pageid_t {
...
#ifdef ENABLE_EXMAP
    if (FLAGS_use_exmap)
      ExmapAlloc(start_pid, required_page_cnt);
#endif
...
bool BufferManager::FixExclusiveOnDiskGC(pageid_t pid, blockid_t bid, u32 p,
                                         u8 pidx) {
  for (auto cnt = 0;; cnt++) {
    if(pid>=virtual_cnt_){return false;}
    if (!sm_->PageIsValid(bid, p, pidx) || !sm_->PIDOffsetIsValid(pid)) {
      return false;
    }
