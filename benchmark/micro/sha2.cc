 ...
 * ~4% slower, but it includes padding, .... may be this extra step adds to the
 * overhead
 *
 * OpenSSL SHA-256
 * https://github.com/openssl/openssl/tree/openssl-3.1.2
 * Same speed, includes all padding, terminating bit blah blah -> Should use
 * this
 *
 * Git SHA-256
...
static void BM_PicoSHA(benchmark::State &state) {
  ...
  for (auto _ : state) {
    picosha2::hash256(payload.begin(), payload.end(), sha2_val);
  }
  state.SetBytesProcessed(static_cast<int64_t>(state.iterations()) *
                          static_cast<int64_t>(state.range(0)));
}
...
static void BM_GcryptSHA(benchmark::State &state) {
  ...
  for (auto _ : state) {
    ...
    memcpy(sha2_val.data(), gcry_md_read(sha256, GCRY_MD_SHA256),
           SHA256_DIGEST_SIZE);
  }
  state.SetBytesProcessed(static_cast<int64_t>(state.iterations()) *
                          static_cast<int64_t>(state.range(0)));
