...
struct FreePageManager {
  FreePageManager(int number_of_pages)
      : no_pages(number_of_pages), alloc_cnt{0}, physical_cnt{0} {}
...
struct FirstFitAllocator : FreePageManager {
  ...
  auto AllocTierPage(int tier, size_t &out_pid) -> bool {
    ...
    for (auto ite = free_pages.begin(); ite != free_pages.end(); ++ite) {
      ...
      if (size >= required_pages) {
        ...
        if (size > required_pages) {
          free_pages.emplace_back(out_pid + required_pages,
                                  size - required_pages);
        }
        return true;
      }
  ...
  std::list<std::pair<size_t, size_t>>
      free_pages; // Linked-list of free pages <start PID, size>
};

struct BitmapFreePages : FreePageManager {
  BitmapFreePages(int number_of_pages)
      : FreePageManager(number_of_pages), page_counter(0) {
    bitmap = std::vector<std::bitset<64>>(
        std::ceil(static_cast<double>(no_pages) / 64), 0);
  }
      ...
      for (; page_counter < no_pages; page_counter++) {
        ...
        if (bitmap[block].test(page_counter % 64)) {
          ...
          for (; (j < no_pages) && (j - page_counter < required_pages); j++) {
            ...
            if (!bitmap[j_block].test(j % 64)) {
              break;
            }
          }
          //  Found a free page range
          if (j - page_counter >= required_pages) {
            for (auto k = page_counter; k < page_counter + required_pages;
                 k++) {
              auto k_block = k / 64;
...
struct TreeFreePages : FreePageManager {
  ...
  auto AllocTierPage(int tier, size_t &out_pid) -> bool override {
    ...
    if (lb != free_pages.end()) {
      ...
      if (free_size > required_pages) {
        InsertRange(out_pid + required_pages, free_size - required_pages);
      }
      physical_cnt += required_pages;
      return true;
    }
  ...
  std::set<std::pair<size_t, size_t>>
      free_pages; // Ordered map of <free page size, start_pid>
  std::unordered_map<size_t, size_t>
      free_start_pid; // Free range: Start PID -> Size
  std::unordered_map<size_t, size_t>
      free_end_pid; // Free range: End PID -> Size
};

template <int HighestTier> struct TieringFreePages : FreePageManager {
