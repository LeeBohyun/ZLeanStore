...
struct BlobRep {
  static constexpr int MAX_TIER =
      16; // 0 -> MAX_TIER (MAX_TIER + 1 first extents),
  ...
  auto MallocSize() -> size_t {
    return sizeof(extent_cnt) + extent_cnt * sizeof(size_t);
  }
};

/**
 * @brief Basically `requested_size` and `append_size` aren't necessary to be
 * perfect i.e. new allocated extents fit the requested size perfectly however,
 * for simplicity, we assume that all allocated extents are filled 100%, that
 * means, `requested_size` is only the lower bound of the real allocated size in
 * this dump simulator and the real size should be the total size of all
 * allocated extents
 */
struct BufferManager {
  ...
  static auto TierIndex(int extent_cnt) {
    return std::min(extent_cnt, BlobRep::MAX_TIER);
  }

  auto BlobCount() -> size_t {
    return std::distance(blob_list.begin(), blob_list.end());
  }
  ...
  void RemoveBlob(int index) {
    auto blob_storage = std::next(blob_list.begin(), index);
    auto blob = reinterpret_cast<BlobRep *>(*blob_storage);
    for (auto idx = 0; idx < blob->extent_cnt; idx++) {
      man->DeallocTierPage(blob->extent_pid[idx], TierIndex(idx));
    }
