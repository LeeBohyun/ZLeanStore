...
TEST(BitmapTest, Basic) {
  ...
  for (auto tier = 0; tier <= 8; tier++) {
    ...
    EXPECT_FALSE(found);
    for (auto idx = 0; idx < no_pages / TIER_SIZE(tier); idx++) {
      man.DeallocTierPage(idx * TIER_SIZE(tier), tier);
    }
    for (auto &map : man.bitmap) {
      EXPECT_EQ(map.to_string(), std::string(64, '1'));
    }
  }
}
...
TEST(BlobAllocWithFirstFit, Basic) {
  ...
  for (auto idx = 1; idx < BlobRep::MAX_TIER; idx++) {
    ...
    EXPECT_TRUE(success);
    EXPECT_EQ(reinterpret_cast<BlobRep *>(buf.blob_list.front())->extent_cnt,
              idx + 1);
    EXPECT_EQ(buf.BlobCount(), 1);
...
TEST(BlobAllocWithBitmap, Basic) {
  ...
  EXPECT_EQ(buf.BlobCount(), 0);
  EXPECT_EQ(man.alloc_cnt, TIER_SIZE(BlobRep::MAX_TIER + 3) - 1);
  for (size_t idx = 0; idx < man.bitmap.size() - 1; idx++) {
    ASSERT_EQ(man.bitmap[idx], std::bitset<64>(ULONG_MAX));
  }
...
TEST(BlobAllocWithTiering, Basic) {
  auto man =
      TieringFreePages<BlobRep::MAX_TIER>(TIER_SIZE(BlobRep::MAX_TIER + 3));
  auto buf = BufferManager(&man);
