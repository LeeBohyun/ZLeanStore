...
#define SET_RANGE_RANDOM(length, datatype)                                     \
  ({                                                                           \
    ...
    for (auto i = start_pos; i < start_pos + size; i++) {                      \
      auto next_pos = std::min(i + SIZEBIT(datatype) - i % SIZEBIT(datatype),  \
                               start_pos + size);                              \
      auto val = (next_pos % SIZEBIT(datatype) == 0)                           \
                     ? std::numeric_limits<datatype>::max()                    \
                     : DUMMY(next_pos, datatype);                              \
      if (i % SIZEBIT(datatype) != 0) {                                        \
        val -= DUMMY(i, datatype);                                             \
      }                                                                        \
      if (rand() % 2 == 0) {                                                   \
...
static void BM_Int8(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (int i = 0; i < NO_OPERATION; i++) {
      SET_RANGE_RANDOM(state.range(0), uint8_t);
    }
  }
}
...
static void BM_Int32(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (int i = 0; i < NO_OPERATION; i++) {
      SET_RANGE_RANDOM(state.range(0), uint32_t);
    }
  }
}
...
static void BM_Int64(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (int i = 0; i < NO_OPERATION; i++) {
      SET_RANGE_RANDOM(state.range(0), uint64_t);
    }
  }
}
...
static void BM_Int128(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (int i = 0; i < NO_OPERATION; i++) {
      SET_RANGE_RANDOM(state.range(0), __int128_t);
    }
  }
}
