...
auto BaseDatabase::StartProfilingThread(const std::string &system_name,
...
PostgresDB::PostgresDB() {
  const auto db_conn = "host=/var/run/postgresql "
                       "dbname=postgres "
                       "user=postgres "
                       "port=5432 "
                       "sslmode=disable";
...
MySQLDB::MySQLDB() {
  db_conn =
      "leanstore:leanstore@unix(/var/run/mysqld/mysqld.sock)/test?charset=utf8";
}

void MySQLDB::PrepareThread() {
  conn.reset(get_driver_instance()->connect(
      "unix:///var/run/mysqld/mysqld.sock", "leanstore", "leanstore"));
  conn->setSchema("test");
...
auto MySQLDB::DatabaseSize() -> float {
  ...
  auto res = stmt->executeQuery(
      "SELECT ALLOCATED_SIZE FROM INFORMATION_SCHEMA.INNODB_TABLESPACES WHERE "
      "NAME='test/YCSB_TABLE';");
