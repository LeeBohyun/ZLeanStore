...
#define READ_ENTRY(path, record)                                               \
  ({                                                                           \
    ...
    if constexpr (requires { (record)->payload; }) {                           \
      assert((record)->payload.MallocSize() == size);                          \
    }                                                                          \
  })
...
// -------------------------------------------------------------------------------------
FilesystemAsDB::FilesystemAsDB(const std::string &root_path, bool enable_fsync,
                               bool track_storage_size)
    : root_dir(fs::path(root_path)), enable_fsync(enable_fsync),
      track_storage_size(track_storage_size) {
...
void FilesystemAsDB::DropCache() {
  ...
  [[maybe_unused]] auto ret =
      system("sudo sh -c \"sync; /usr/bin/echo 3 > /proc/sys/vm/drop_caches\"");
  assert(ret >= 0);
  LOG_INFO("Complete dropping page cache");
}

auto FilesystemAsDB::StartProfilingThread(std::atomic<bool> &keep_running,
  ...
  return std::thread([&]() {
    ...
    while (keep_running.load()) {
      ...
      if (track_storage_size) {
        ...
        auto db_size =
            static_cast<float>((buf.f_blocks - buf.f_bavail) * buf.f_frsize) /
            (1024 * 1024 * 1024);
        std::printf("filesystem,%lu,%lu,%.4f,%lu,%lu\n", cnt++, progress,
                    db_size, buf.f_blocks, buf.f_bavail);
      } else {
...
thread_local std::unique_ptr<EVP_MD_CTX, FilesystemAsDB::EvlDeleter>
    FilesystemAdapter<RecordBase>::sha_context =
        std::unique_ptr<EVP_MD_CTX, FilesystemAsDB::EvlDeleter>{
            EVP_MD_CTX_create()};

template <class RecordBase>
FilesystemAdapter<RecordBase>::FilesystemAdapter(FilesystemAsDB *db)
    : db_(db) {}

template <class RecordBase>
auto FilesystemAdapter<RecordBase>::ReadPayload(
    const fs::path &entry, std::unique_ptr<uint8_t[], FreeDelete> &out_payload)
    -> size_t {
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::FileStat(
...
/**
 * @brief Extra op to create a fairer comparison between Filesystem and
 * LeanStore This includes:
 * - sha256 calculation
 */
template <class RecordBase>
void FilesystemAdapter<RecordBase>::ExtraOperator(
...
// -------------------------------------------------------------------------------------
template <class RecordBase>
void FilesystemAdapter<RecordBase>::Scan(
  ...
  for (const auto &entry : fs::directory_iterator(db_->root_dir)) {
    ...
    if (filename.compare(key_str) <= 0) {
      ...
      if (!found_record_cb(s_key, *record)) {
        break;
      }
    }
  }
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::ScanDesc(
    const typename RecordBase::Key &key,
    const typename Adapter<RecordBase>::FoundRecordFunc &found_record_cb) {
  auto key_str = key.String();
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::Insert(
  ...
  ExtraOperator(
      {reinterpret_cast<const uint8_t *>(&record), record.PayloadSize()});
  if (db_->enable_fsync) {
    if (rand() % FLAGS_fs_fsync_rate == 0) {
      sync();
    }
  }
  close(fd);
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::InsertRawPayload(
    const typename RecordBase::Key &r_key, std::span<const uint8_t> record) {
  auto path = db_->root_dir / r_key.String();
  auto fd = open(path.c_str(), O_RDWR | O_CREAT, S_IRWXU);
  assert(fd >= 0);
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::Update(
    const typename RecordBase::Key &r_key, const RecordBase &record) {
  auto path = db_->root_dir / r_key.String();
  auto fd = open(path.c_str(), O_RDWR | O_CREAT, S_IRWXU);
  assert(fd >= 0);
...
template <class RecordBase>
auto FilesystemAdapter<RecordBase>::LookUp(
    const typename RecordBase::Key &r_key,
...
template <class RecordBase>
void FilesystemAdapter<RecordBase>::UpdateInPlace(
    const typename RecordBase::Key &r_key,
...
template <class RecordBase>
auto FilesystemAdapter<RecordBase>::Erase(const typename RecordBase::Key &r_key)
    -> bool {
  auto path = db_->root_dir / r_key.String();
...
auto FilesystemAdapter<RecordBase>::Count() -> uint64_t {
  return std::distance(fs::directory_iterator(db_->root_dir),
