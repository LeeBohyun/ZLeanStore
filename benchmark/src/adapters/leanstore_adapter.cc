...
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::SetComparisonOperator(
    leanstore::ComparisonOperator cmp) {
  tree_->SetComparisonOperator(db_->RetrieveComparisonFunc(cmp));
}

template <class RecordBase>
void LeanStoreAdapter<RecordBase>::ScanImpl(
  ...
  auto read_cb = [&](std::span<u8> key, std::span<u8> payload) -> bool {
    ...
    return found_record_cb(
        typed_key, *reinterpret_cast<const RecordBase *>(payload.data()));
  };
...
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::Scan(
...
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::ScanDesc(
    const typename RecordBase::Key &key,
    const typename Adapter<RecordBase>::FoundRecordFunc &found_record_cb) {
...
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::Insert(const typename RecordBase::Key &r_key,
                                          const RecordBase &record) {
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
  tree_->Insert({key, len},
                {reinterpret_cast<const u8 *>(&record), record.PayloadSize()});
}

template <class RecordBase>
void LeanStoreAdapter<RecordBase>::InsertRawPayload(
    const typename RecordBase::Key &r_key, std::span<const u8> record) {
  Ensure(record.size() ==
         reinterpret_cast<const RecordBase *>(record.data())->PayloadSize());
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
...
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::Update(const typename RecordBase::Key &r_key,
                                          const RecordBase &record) {
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
  tree_->Update({key, len},
                {reinterpret_cast<const u8 *>(&record), record.PayloadSize()});
}

template <class RecordBase>
void LeanStoreAdapter<RecordBase>::UpdateRawPayload(
    const typename RecordBase::Key &r_key, std::span<const u8> record) {
  Ensure(record.size() ==
         reinterpret_cast<const RecordBase *>(record.data())->PayloadSize());
  u8 key[RecordBase::MaxFoldLength()];
...
template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::LookUp(
    const typename RecordBase::Key &r_key,
    const typename Adapter<RecordBase>::AccessRecordFunc &fn) -> bool {
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
  bool success = tree_->LookUp({key, len}, [&](std::span<u8> pl) {
    fn(*reinterpret_cast<RecordBase *>(pl.data()));
  });
  return success;
}

template <class RecordBase>
void LeanStoreAdapter<RecordBase>::UpdateInPlace(
    const typename RecordBase::Key &r_key,
    const typename Adapter<RecordBase>::ModifyRecordFunc &fn) {
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
  tree_->UpdateInPlace({key, len}, [&](std::span<u8> payload) {
    fn(*reinterpret_cast<RecordBase *>(payload.data()));
  });
}

template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::Erase(const typename RecordBase::Key &r_key)
    -> bool {
  u8 key[RecordBase::MaxFoldLength()];
  auto len = RecordBase::FoldKey(key, r_key);
...
template <class RecordBase> auto LeanStoreAdapter<RecordBase>::Count() -> u64 {
...
// template <class RecordBase>
// auto LeanStoreAdapter<RecordBase>::RelationPageCount() -> float {
//   return tree_->SizeInMB();
// }

// hack functions to update in a page level
template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::UpdatePID(pageid_t pid, u8 *payload)
    -> bool {
  return tree_->UpdatePID(pid, payload);
}

template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::ReadPID(pageid_t pid) -> bool {
  return tree_->ReadPID(pid);
}

template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::AllocPID(pageid_t pid, u8 *payload) -> bool {
  return tree_->AllocPID(pid, payload);
}
 ...
 */
template <class RecordBase>
void LeanStoreAdapter<RecordBase>::MiniTransactionWrapper(
    const std::function<void()> &op, wid_t wid) {
...
template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::RegisterBlob(std::span<u8> blob_payload,
                                                std::span<u8> prev_blob,
                                                bool likely_grow)
    -> std::span<const u8> {
  auto prev_btup =
      (prev_blob.empty())
          ? nullptr
          : reinterpret_cast<leanstore::BlobState *>(prev_blob.data());
  return db_->CreateNewBlob(blob_payload, prev_btup, likely_grow);
}

template <class RecordBase>
void LeanStoreAdapter<RecordBase>::LoadBlob(
    u8 *blob_handler, const std::function<void(std::span<const u8>)> &read_cb,
    bool partial_load) {
...
template <class RecordBase>
auto LeanStoreAdapter<RecordBase>::LookUpBlob(
    std::span<u8> blob_payload,
    const typename Adapter<RecordBase>::AccessRecordFunc &fn) -> bool {
  auto success = tree_->LookUpBlob(
      blob_payload,
      db_->RetrieveComparisonFunc(leanstore::ComparisonOperator::BLOB_LOOKUP),
      [&](std::span<u8> payload) {
...
// For flashbench
template struct LeanStoreAdapter<flash::FlashRelation<BytesPayload<1200>, 0>>;
