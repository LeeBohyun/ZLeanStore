...
WiredTigerDB::WiredTigerDB() {
  std::string config_string("create, direct_io=[data, log, checkpoint], "
                            "log=(enabled=false), statistics_log=(wait=1), "
                            "statistics=(all, clear), session_max=2000, "
  ...
  assert(ret == 0);
  ret = wiredtiger_open(FLAGS_db_path.c_str(), nullptr, config_string.c_str(),
                        &conn);
  WTErrorCheck(ret);
}
...
WiredTigerAdapter<RecordBase>::WiredTigerAdapter(WiredTigerDB &map)
  ...
  int ret =
      map_.session->create(map_.session, relation_name_.c_str(),
                           "key_format=S,value_format=S,memory_page_max=10M");
  WTErrorCheck(ret);
}
...
void WiredTigerAdapter<RecordBase>::Insert(
  ...
  if (ret == WT_ROLLBACK) {
    WTErrorCheck(map_.session->rollback_transaction(map_.session, nullptr));
  }
  WTErrorCheck(ret);
}
...
void WiredTigerAdapter<RecordBase>::UpdateInPlace(
  ...
  RecordBase record;
  LookUp(r_key, [&](const RecordBase &rec) { record = rec; });
  fn(record);
  Insert(r_key, record);
}
