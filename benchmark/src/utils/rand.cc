...
MersenneTwister::MersenneTwister(uint64_t seed) : mti_(NN + 1) {
...
void MersenneTwister::Init(uint64_t seed) {
  ...
  for (mti_ = 1; mti_ < NN; mti_++) {
    mt_[mti_] =
        (6364136223846793005ULL * (mt_[mti_ - 1] ^ (mt_[mti_ - 1] >> 62)) +
         mti_);
  }
}
...
ZipfGenerator::ZipfGenerator(double theta, uint64_t n_elements)
    : n_elements_(n_elements) {

  norm_c_ = 0.0;

  for (uint64_t i = 1; i <= n_elements; ++i) {
    norm_c_ += 1.0 / pow(static_cast<double>(i), theta);
  }
  norm_c_ = 1.0 / norm_c_;
  ...
  for (uint64_t i = 1; i <= n_elements; ++i) {
    sum_prob_[i] =
        sum_prob_[i - 1] + norm_c_ / pow(static_cast<double>(i), theta);
  }
}

auto ZipfGenerator::Rand() -> uint64_t {
  double z;
...
template <typename T> inline auto RandomGenerator::GetRand(T min, T max) -> T {
...
void RandomGenerator::GetRandRepetitiveString(uint8_t *dst, uint64_t rep_size,
                                              uint64_t size) {
  if (rep_size > size) {
    rep_size = size;
  }
...
auto UniformRand(Integer low, Integer high) -> Integer {
  return Rand(high - low + 1) + low;
}

// Generates a uniform random double in the range [min, max]
auto GetUniformRandDouble(double min, double max) -> double {
  uint64_t rand = mt_generator.Rand();
  double fraction = static_cast<double>(rand) / static_cast<double>(UINT64_MAX);
...
auto NonUniformRand(Integer a, Integer x, Integer y, Integer c) -> Integer {
  ...
  // in standard: NURand(A, x, y) = (((random(0, A) | random(x, y)) + C) % (y -
  // x + 1)) + x return (((rnd(a + 1) | rnd((y - x + 1) + x)) + 42) % (y - x +
  // 1)) + x;
