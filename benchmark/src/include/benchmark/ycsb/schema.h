...
static constexpr uint32_t SUPPORTED_PAYLOAD_SIZE[] = {120, 1200, 102400,
                                                      1048576, 10485760};
...
template <typename TablePayload, int TypeIdValue> struct Relation {
  ...
  auto PayloadSize() const -> uint32_t {
    return sizeof(Relation<TablePayload, TypeIdValue>);
  }
...
struct BlobStateRelation {
  ...
  leanstore::BlobState my_payload;

  // -------------------------------------------------------------------------------------
  auto PayloadSize() const -> uint32_t { return my_payload.MallocSize(); }
