...
struct WikipediaReadOnly {
  ...
  explicit WikipediaReadOnly(bool enable_blob_rep, Params &&...params)
      : relation(AdapterType<Relation>(std::forward<Params>(params)...)),
        enable_blob_rep(enable_blob_rep), inrow_count(0), offrow_count(0) {
    ...
    for (uint32_t idx = 0; idx < characteristic.size(); idx++) {
      dist[idx] = (idx > 0) ? dist[idx - 1] + characteristic[idx].second
                            : characteristic[idx].second;
    }
  }
  ...
  // -------------------------------------------------------------------------------------
  void
  LoadInitialDataWithoutBlobRep(const tbb::blocked_range<UInteger> &range) {
  ...
  void LoadInitialData(const tbb::blocked_range<UInteger> &range) {
    ...
    for (auto key = range.begin(); key < range.end(); key++) {
      ...
      auto payload_sz = characteristic[key - 1].first;
      std::unique_ptr<uint8_t[], FreeDelete> payload(
          reinterpret_cast<uint8_t *>(malloc(payload_sz)));
      RandomGenerator::GetRandRepetitiveString(payload.get(), 100UL,
                                               payload_sz);

      if (payload_sz > MAX_INROW_SIZE) {
        auto blob_rep =
            relation.RegisterBlob({payload.get(), payload_sz}, {}, false);
  ...
  void ExecuteReadOnlyTxn() {
    ...
    auto found = relation.LookUp({access_key}, [&](const Relation &rec) {
      ...
      std::memcpy(tuple, const_cast<Relation &>(rec).payload,
                  rec.PayloadSize());
    });
    Ensure(found);
    if (is_offrow) {
