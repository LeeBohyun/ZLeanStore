...
struct BlobStateIndex {
  ...
  static auto FoldKey(uint8_t *out, const Key &key) -> uint16_t {
    auto size =
        reinterpret_cast<const leanstore::BlobState *>(key.value)->MallocSize();
  ...
  static auto UnfoldKey(const uint8_t *in, Key &key) -> uint16_t {
    auto size =
        reinterpret_cast<const leanstore::BlobState *>(in)->MallocSize();
    std::memcpy(key.value, in, size);
    return size;
  }

  static auto MaxFoldLength() -> uint32_t {
    return leanstore::BlobState::MAX_MALLOC_SIZE;
  }
};

template <uint32_t PrefixLength> struct PrefixIndex {
  ...
  auto PayloadSize() const -> uint32_t {
    return sizeof(article_id) + SHA2_LENGTH;
  }
  ...
  static auto MaxFoldLength() -> uint32_t {
    return PrefixLength + sizeof(Key::prefix_length);
  }
};

using FileRelation =
    benchmark::FileRelation<0, 1775872>; // Max payload is retrieved from the
                                         // traces
