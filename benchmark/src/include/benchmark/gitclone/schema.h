...
struct Operator {
  ...
  inline static std::unordered_map<std::string, Type> type_resolver = {
      {"SYS_openat", Operator::Type::OPEN},
  ...
  Operator(Type t, int fd, size_t sz, int flags)
      : op(t), file_desc(fd), size(sz), flags(flags) {}
};
