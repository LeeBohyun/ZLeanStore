...
template <template <typename> class AdapterType, typename SystemRelation,
          typename TemplateRelation, typename ObjectRelation>
struct GitCloneWorkload {
  ...
  explicit GitCloneWorkload(bool enable_blob_rep, Params &&...params)
      : system_files(
            AdapterType<SystemRelation>(std::forward<Params>(params)...)),
        template_files(
            AdapterType<TemplateRelation>(std::forward<Params>(params)...)),
    ...
    while (in.read_row(op_type, file_desc, size, flags)) {
      operators.emplace_back(Operator::type_resolver[op_type], file_desc, size,
                             flags);
    }
  }
  ...
  void MainPhaseImplementation(
      const std::function<void(const Operator &op, uint64_t &next_key,
  ...
  void SetupFilesWithoutBlobRep(const R &file_range,
    ...
    for (auto key = 0UL; key < file_range.size(); key++) {
      ...
      RandomGenerator::GetRandRepetitiveString(
          reinterpret_cast<uint8_t *>(object->payload.data), 100UL,
          payload_size);
  ...
  void InitializationPhaseWithoutBlobRep() {
    ...
    for (auto key = 0UL; key < SYSTEM_FILE_INFO.size(); key++) {
      payload = nullptr;
      auto found =
          system_files.LookUp({key + 1}, [&](const SystemRelation &rec) {
            payload.reset(
                reinterpret_cast<uint8_t *>(malloc(rec.payload.length)));
    ...
    for (auto key = 0UL; key < TEMPLATE_FILE_INFO.size(); key++) {
      payload = nullptr;
      auto found =
          template_files.LookUp({key + 1}, [&](const TemplateRelation &rec) {
            payload.reset(
                reinterpret_cast<uint8_t *>(malloc(rec.PayloadSize())));
            std::memcpy(payload.get(), &rec, rec.PayloadSize());
          });
      Ensure(found);
      // Don't construct a new FileRelation() here to prevent extra copy
      objects.Insert({key + 1},
                     *reinterpret_cast<ObjectRelation *>(payload.get()));
    }
  }
  ...
  void MainPhaseWithoutBlobRep() {
    ...
    MainPhaseImplementation(
        [&](const Operator &op, uint64_t &next_key,
            std::unique_ptr<uint8_t[], FreeDelete> &payload) {
          switch (op.op) {
          case Operator::Type::OPEN:
            payload = nullptr;
            break;
          case Operator::Type::WRITE: {
            payload.reset(reinterpret_cast<uint8_t *>(
                malloc(sizeof(Varchar<0>) + op.size)));
  ...
  template <std::ranges::random_access_range R, typename Relation>
  void SetupExistingFiles(const R &file_range,
                          AdapterType<Relation> &relation) {
    ...
    for (auto key = 0UL; key < file_range.size(); key++) {
      auto &[file, payload_size] = file_range[key];
      std::unique_ptr<uint8_t[], FreeDelete> payload(
          reinterpret_cast<uint8_t *>(malloc(payload_size)));
      RandomGenerator::GetRandRepetitiveString(payload.get(), 100UL,
                                               payload_size);

      if (enable_blob_rep && payload_size > MAX_INROW_SIZE) {
        auto blob_rep =
            relation.RegisterBlob({payload.get(), payload_size}, {}, false);
        record = new (tmp) Relation{0, blob_rep};
      } else {
        record =
            new (tmp) Relation{payload_size, {payload.get(), payload_size}};
      }
  ...
  /**
   * @brief Read file `key` from file relation `relation`, and store the content
   * into `out_tuple`, or `out_payload` if the file is not fit into a single row
   */
  template <typename Relation>
  void ReadFilePayload(const uint64_t key, AdapterType<Relation> &relation,
                       bool &out_is_offrow, uint8_t *out_tuple,
                       std::unique_ptr<uint8_t[]> &out_payload,
    ...
    auto found = relation.LookUp({key}, [&](const Relation &rec) {
      ...
      std::memcpy(out_tuple, const_cast<Relation &>(rec).payload,
                  rec.PayloadSize());
    });
    Ensure(found);
    if (out_is_offrow) {
      relation.LoadBlob(
          ...
          [&](std::span<const u8> blob_data) {
            out_payload.reset(
                reinterpret_cast<uint8_t *>(malloc(blob_data.size())));
  ...
  void NewObject(bool is_offrow, uint64_t key, uint8_t *payload,
                 size_t payload_size) {
    Ensure(enable_blob_rep);
  ...
  void InitializationPhase() {
    ...
    for (auto key = 0UL; key < SYSTEM_FILE_INFO.size(); key++) {
      ...
      ReadFilePayload(key + 1, system_files, is_offrow, tuple, payload,
                      payload_size);
    }
    ...
    for (auto key = 0UL; key < TEMPLATE_FILE_INFO.size(); key++) {
      std::unique_ptr<uint8_t[]> payload = nullptr;
      ReadFilePayload(key + 1, template_files, is_offrow, tuple, payload,
                      payload_size);
      NewObject(is_offrow, key + 1, (is_offrow) ? payload.get() : &tuple[0],
                payload_size);
    }
  }
        ...
        [&](const Operator &op, uint64_t &next_key,
          ...
          case Operator::Type::FSTAT: {
            [[maybe_unused]] auto found =
                objects.LookUp({next_key}, [&](const auto &rec) {
