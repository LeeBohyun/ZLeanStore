...
struct WarehouseType {
  ...
  static auto UnfoldKey(const uint8_t *in, WarehouseType::Key &key)
      -> uint16_t {
...
struct DistrictType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::d_w_id) + sizeof(Key::d_id);
  }
};
...
struct CustomerType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::c_w_id) + sizeof(Key::c_d_id) + sizeof(Key::c_id);
  }
};
...
struct CustomerWDCType {
  ...
  static auto FoldKey(uint8_t *out, const CustomerWDCType::Key &key)
      -> uint16_t {
    auto pos = Fold(out, key.c_w_id);
    pos += Fold(out + pos, key.c_d_id);
  ...
  static auto UnfoldKey(const uint8_t *in, CustomerWDCType::Key &key)
      -> uint16_t {
    auto pos = Unfold(in, key.c_w_id);
    pos += Unfold(in + pos, key.c_d_id);
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::c_w_id) + sizeof(Key::c_d_id) + sizeof(Key::c_last) +
           sizeof(Key::c_first);
  }
};
...
struct HistoryType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::h_t_id) + sizeof(Key::h_pk);
  }
};
...
struct NewOrderType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::no_w_id) + sizeof(Key::no_d_id) +
           sizeof(Key::no_o_id);
  }
};
...
struct OrderType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::o_w_id) + sizeof(Key::o_d_id) + sizeof(Key::o_id);
  }
};
...
struct OrderWDCType {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::o_w_id) + sizeof(Key::o_d_id) + sizeof(Key::o_c_id) +
           sizeof(Key::o_id);
  }
};
...
struct OrderLineType {
  ...
  static auto UnfoldKey(const uint8_t *in, OrderLineType::Key &key)
      -> uint16_t {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::ol_w_id) + sizeof(Key::ol_d_id) +
...
struct StockType {
  ...
  Numeric s_quantity;
  // holds a character string representing the stock quantity for a specific
  // item in the first district (or region) of a warehouse.
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::s_w_id) + sizeof(Key::s_i_id);
  }
};
