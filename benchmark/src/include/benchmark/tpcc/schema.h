...
struct WarehouseType {
  ...
  static auto UnfoldKey(const uint8_t *in,
                        WarehouseType::Key &key) -> uint16_t {
...
struct CustomerWDCType {
  ...
  static auto FoldKey(uint8_t *out,
                      const CustomerWDCType::Key &key) -> uint16_t {
    auto pos = Fold(out, key.c_w_id);
    pos += Fold(out + pos, key.c_d_id);
  ...
  static auto UnfoldKey(const uint8_t *in,
                        CustomerWDCType::Key &key) -> uint16_t {
    auto pos = Unfold(in, key.c_w_id);
    pos += Unfold(in + pos, key.c_d_id);
...
struct OrderLineType {
  ...
  static auto UnfoldKey(const uint8_t *in,
                        OrderLineType::Key &key) -> uint16_t {
