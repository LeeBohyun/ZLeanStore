...
template <template <typename> class AdapterType> struct TPCCWorkload {
  ...
  static constexpr std::array NAME_PARTS = {"Bar",   "OUGHT", "ABLE", "PRI",
                                            "PRES",  "ESE",   "ANTI", "CALLY",
                                            "ATION", "EING"};
  ...
  // whether we enable index on Order(warehouse-id, district-id, customer-id,
  // order-id)
  ...
  // batch size for deletion, configurable by
  Integer window_size = 0; // default

  // maintained per warehouse for batch deletion of ol and o relation
  struct order_per_d {
    Integer max_o_id;           // newest order id
    Integer min_delivered_o_id; // the oldest order id that has been delivered
    Integer max_delivered_o_id; // the oldest order id that has been delivered
    Integer max_deleted_o_id;   // the oldest order id that has been delivered
    std::mutex mutex;
  };

  order_per_d **order_metadata_ = new order_per_d *[warehouse_count + 1];

  // maintained per worker thread for batch deletion of history relation
  struct hist_per_t {
    Integer history_pk_counter; // newest hist_pk
    Integer
        last_deleted_history_pk; // the oldest order id that has been delivered
    std::mutex mutex;
  };
  hist_per_t *history_metadata_ = new hist_per_t[FLAGS_worker_count + 1];

  // Random utilities
  auto GenName(Integer id) -> Varchar<16> {
    return Varchar<16>(NAME_PARTS[(id / 100) % 10]) ||
           Varchar<16>(NAME_PARTS[(id / 10) % 10]) ||
  ...
  // Query c_id from either c_id (check if customer exists) or by c_last (c's
  // last name)
  void QueryCustomerInfo(Integer w_id, Integer d_id,
  ...
  TPCCWorkload(Integer warehouse_count, bool enable_order_wdc_index,
               bool enable_cross_warehouses,
               bool manually_handle_isolation_anomalies, Integer window_sz,
               Params &&...params)
      : warehouse(AdapterType<WarehouseType>(std::forward<Params>(params)...)),
        ...
        customer_wdc(
            AdapterType<CustomerWDCType>(std::forward<Params>(params)...)),
        ...
        manually_handle_isolation_anomalies(
            manually_handle_isolation_anomalies),
  ...
  void NewOrder(Integer w_id, Integer d_id, Integer c_id,
                ...
                const std::vector<Integer> &item_ids,
  ...
  // -------------------------------------------------------------------------------------
  void OrderStatus(Integer w_id, Integer d_id,
                   std::variant<Integer, Varchar<16>> c_info);
  void DoOrderStatusRandom(Integer w_id);
  // -------------------------------------------------------------------------------------
  void Payment(Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id,
               Timestamp h_date, Numeric h_amount, Timestamp datetime,
               std::variant<Integer, Varchar<16>> c_info);
  void DoPaymentRandom(Integer w_id);

  /* 6th transaction, batch delete*/
  void DoBatchDelete(Integer w_id);
  /* end */
  // -------------------------------------------------------------------------------------
  // Initial data loader
