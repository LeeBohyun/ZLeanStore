...
struct FilesystemAsDB {
  ...
  FilesystemAsDB(const std::string &root_path, bool enable_fsync,
                 bool track_storage_size = false);
  ~FilesystemAsDB() = default;

  void DropCache();
  auto DatabaseSize() -> float;
  auto StartProfilingThread(std::atomic<bool> &keep_running,
...
public:
  static thread_local std::unique_ptr<EVP_MD_CTX, FilesystemAsDB::EvlDeleter>
      sha_context;
  ...
  // -------------------------------------------------------------------------------------
  auto ReadPayload(const fs::path &entry,
                   std::unique_ptr<uint8_t[], FreeDelete> &out_payload)
      -> size_t;
