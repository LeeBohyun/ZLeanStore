...
template <class RecordBase> struct LeanStoreAdapter : Adapter<RecordBase> {
private:
  ...
  void
  ScanImpl(const typename RecordBase::Key &r_key,
...
public:
  ...
  auto UpdatePID(pageid_t pid, u8 *payload) -> bool;
  auto ReadPID(pageid_t pid) -> bool;
  auto AllocPID(pageid_t pid, u8 *payload) -> bool;
  ...
  void InsertRawPayload(const typename RecordBase::Key &r_key,
                        std::span<const u8> record);
  void Update(const typename RecordBase::Key &r_key,
              const RecordBase &record) override;
  void UpdateRawPayload(const typename RecordBase::Key &r_key,
                        std::span<const u8> record);
  ...
  void LoadBlob(u8 *blob_handler,
                const std::function<void(std::span<const u8>)> &read_cb,
                bool partial_load);
