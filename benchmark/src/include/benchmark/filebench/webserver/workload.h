...
#define RELATION_LOADS_BLOB(relation, key, likely_grow)                        \
  ({                                                                           \
    ...
    auto r_span =                                                              \
        (relation).RegisterBlob({payload, file_size}, {}, likely_grow);        \
 ...
 */
template <template <typename> class AdapterType, class HTMLRelation,
          class LogRelation>
struct WebserverWorkload {
  ...
  explicit WebserverWorkload(UInteger number_of_html_files, Params &&...params)
      : html_relation(
            AdapterType<HTMLRelation>(std::forward<Params>(params)...)),
    ...
    rand = std::gamma_distribution<>(GAMMA_DIST_GAMMA,
                                     MEAN_FILE_SIZE / GAMMA_DIST_GAMMA);
  }
  ...
  void LoadHTMLData(const tbb::blocked_range<Integer> &range) {
    for (auto key = range.begin(); key < range.end(); key++) {
      RELATION_LOADS_BLOB(html_relation, key, false);
    }
  }
  ...
  void ExecuteTransaction() {
    ...
    for (u32 i = 0; i < FLAGS_webserver_file_read_per_txn; i++) {
      ...
      auto found =
          html_relation.LookUp({access_key}, [&](const HTMLRelation &rec) {
            blob_rep_size = rec.PayloadSize();
            std::memcpy(blob_rep,
                        const_cast<HTMLRelation &>(rec).payload.Data(),
                        blob_rep_size);
          });
      Ensure(found);
    ...
    log_relation.LookUp({ws_thread_id}, [&](const LogRelation &rec) {
      blob_rep_size = rec.PayloadSize();
      std::memcpy(blob_rep, const_cast<LogRelation &>(rec).payload.Data(),
                  blob_rep_size);
    });
    log_relation.RegisterBlob({ws_payload, FLAGS_webserver_log_entry_size},
                              {blob_rep, blob_rep_size}, true);
  }

  // -------------------------------------------------------------------------------------
private:
  auto GetFileSize() -> uint32_t {
    return static_cast<uint32_t>(std::round(rand(generator))) %
