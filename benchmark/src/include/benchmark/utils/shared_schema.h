...
template <int TypeID, uint32_t MaxPayload> struct FileRelation {
  ...
  struct Key {
    ...
    auto String() const -> std::string {
      return fmt::format("{}_{}", TypeID, my_key);
    }

    void FromString(const std::string &suffix) {
      my_key = std::stoi(suffix.substr(suffix.find('_'), suffix.size()));
    }
  };
