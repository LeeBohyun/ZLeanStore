...
struct RelationTest {
  ...
  static auto MaxFoldLength() -> uint32_t {
    return 0 + sizeof(Key::primary_id);
  }
};
 ...
 *  which requires an attribute which expresses the size of this flexible array
 * member In the below example, the corresponding attributes are `uint8_t
 * data[]` and `Integer size`, respectively
 *
 * To allocate such object, a char[] should be allocated first, either on stack
 * or heap, then use placement new to allocate VariableSizeRelation() on that
 * storage Check `test/benchmark/test_leanstore_adapter.cc` for more information
 */
struct VariableSizeRelation {
  ...
  static auto FoldKey(uint8_t *out,
                      const VariableSizeRelation::Key &key) -> uint16_t {
    auto pos = Fold(out, key.primary_id);
    return pos;
  }

  static auto UnfoldKey(const uint8_t *in,
                        VariableSizeRelation::Key &key) -> uint16_t {
    auto pos = Unfold(in, key.primary_id);
    return pos;
  }
