...
private:
  ...
  uint64_t n_elements_;          // Number of elements
  std::vector<double> sum_prob_; // pre calculate the sum probabilities
public:
  ZipfGenerator(double theta, uint64_t n_elements);
  auto Rand() -> uint64_t;
};
...
public:
  ...
  template <typename T> static inline auto GetRand(T min, T max) -> T;
  static void GetRandString(uint8_t *dst, uint64_t size);
  static void GetRandRepetitiveString(uint8_t *dst, uint64_t rep_size,
                                      uint64_t size);
};

auto Rand(Integer n) -> Integer;                        // [0, n)
auto UniformRand(Integer low, Integer high) -> Integer; // [low, high]
auto UniformRandExcept(Integer low, Integer high,
                       Integer v) -> Integer; // [low, high]
...
template <int MaxLength>
auto RandomStringNumber(Integer min_len,
                        Integer max_len) -> Varchar<MaxLength> {
  assert(max_len <= MaxLength);
  Integer str_len = Rand(max_len - min_len + 1) + min_len;
  Varchar<MaxLength> result;
  Integer len = UniformRand(0, 24);
  for (auto index = 0; index < str_len; index++) {
    if (index < len) {
      Integer i = UniformRand(30, 39);
      result.Append(i);
    } else {
      result.Append(0);
    }
...
template <int Length> auto RandomNumberString() -> Varchar<Length> {
  Varchar<16> result;
  for (Integer i = 0; i < Length; i++) {
    result.Append(48 + Rand(10));
  }
  return result;
}