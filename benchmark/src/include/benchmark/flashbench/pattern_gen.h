...
public:
  PatternGen(std::string pattern, std::string zones_str,
             uint64_t req_size = 4096)
      : pattern_string(std::move(pattern)),
        pattern(StringToPattern(pattern_string)),
        zones_string(std::move(zones_str)),
        ...
        logical_pages(FLAGS_initial_dataset_size_gb * leanstore::GB /
                      GLOBAL_BLOCK_SIZE),
        alpha(0.0), beta(0.0), skew_factor(FLAGS_zipf_factor),
        total_writes(logical_pages * FLAGS_exec_writes_pct *
                     (100 / (100 - FLAGS_read_ratio))),
        page_size(req_size), request_size(GLOBAL_BLOCK_SIZE), sector_size(0),
        seq(0), rnd_page(0, logical_pages - 1), update_pattern(),
        trace_accessed_pages(false), zipf_sampler(logical_pages, skew_factor) {
    Init();
  }

  PatternGen(Pattern pattern, uint64_t logical_pages, double skew_factor = 0.0,
             bool shuffle = true)
      : pattern_string(), pattern(pattern), zones_string(), shuffle(shuffle),
        logical_pages(logical_pages), alpha(0.0), beta(0.0),
        skew_factor(skew_factor), total_writes(10000),
        page_size(GLOBAL_BLOCK_SIZE), request_size(GLOBAL_BLOCK_SIZE),
        sector_size(0), seq(0), rnd_page(0, logical_pages - 1),
        update_pattern(), trace_accessed_pages(false),
