...
public:
  ...
  virtual void LoadInitialData(const tbb::blocked_range<Integer> &range) = 0;
...
struct FlashWorkload : public FlashWorkloadInterface {
  ...
  void InitializePatternGen() {
    ...
    pg = std::make_unique<PatternGen>(workload_pattern, zone_pattern,
                                      GLOBAL_BLOCK_SIZE);
  ...
  void LoadInitialData(const tbb::blocked_range<Integer> &range) override {
    ...
    for (auto key = range.begin(); key < range.end(); key++) {
      ...
      relation.AllocPID(payload.get());
    }
  }

  void ExecuteTransaction() override {
