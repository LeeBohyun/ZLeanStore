...
auto main(int argc, char **argv) -> int {
  ...
  auto webserver = std::make_unique<filebench::webserver::WebserverWorkload<
      LeanStoreAdapter, filebench::webserver::BlobStateRelation<0>,
      filebench::webserver::BlobStateRelation<1>>>(FLAGS_webserver_no_files,
                                                   *db);
  ...
  tbb::parallel_for(
      tbb::blocked_range<Integer>(1, FLAGS_webserver_no_files + 1),
  ...
  db->worker_pool.ScheduleSyncJob(0, [&]() {
    db->StartTransaction();
    LOG_DEBUG("File count: %lu - Log files count: %lu",
              webserver->html_relation.Count(),
              webserver->log_relation.Count());
    assert(webserver->html_relation.Count() ==
           static_cast<u64>(FLAGS_webserver_no_files));
  ...
  std::this_thread::sleep_for(
      std::chrono::seconds(FLAGS_webserver_exec_seconds));
  keep_running = false;
  db->Shutdown();
  LOG_INFO("Space used: %.4f GB - WAL size: %.4f GB", db->AllocatedSize(),
           db->WALSize());
}
