...
auto main(int argc, char **argv) -> int {
  ...
  db->txn->exec0(
      "CREATE TABLE YCSB_TABLE (my_key INTEGER PRIMARY KEY, my_payload TEXT);");
  db->txn->exec0(
      "ALTER TABLE YCSB_TABLE ALTER COLUMN my_payload SET STORAGE EXTERNAL;");
    ...
    threads.emplace_back([&]() {
      ...
      while (keep_running.load()) {
        ...
        if (RandomGenerator::GetRandU64(0, 100) <= FLAGS_ycsb_read_ratio) {
          auto res = db->txn->exec_params(
              "SELECT * FROM YCSB_TABLE WHERE my_key = $1", access_key);
        } else {
          auto payload_sz = ycsb::YCSBWorkloadInterface::PayloadSize();
          RandomGenerator::GetRandRepetitiveString(
              reinterpret_cast<uint8_t *>(payload.data()), 100UL, payload_sz);
