...
auto main(int argc, char **argv) -> int {
  ...
  if (FLAGS_ycsb_max_payload_size == ycsb::BLOB_NORMAL_PAYLOAD) {
    stmt->execute(fmt::format("CREATE TABLE YCSB_TABLE (my_key INTEGER PRIMARY "
                              "KEY, my_payload VARCHAR({}));",
                              FLAGS_ycsb_max_payload_size));
  } else {
    stmt->execute("CREATE TABLE YCSB_TABLE (my_key INTEGER PRIMARY KEY, "
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<Integer> &range) {
        ...
        for (auto key = range.begin(); key < range.end(); key++) {
          ...
          pstm.reset(db->conn->prepareStatement(
