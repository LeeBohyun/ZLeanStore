...
auto main(int argc, char **argv) -> int {
  ...
  db->ui << fmt::format("CREATE TABLE YCSB_TABLE (my_key INTEGER PRIMARY KEY, "
                        "my_payload VARCHAR({}));",
  ...
  tbb::global_control c(tbb::global_control::max_allowed_parallelism,
                        FLAGS_worker_count);
  tbb::parallel_for(
      tbb::blocked_range<Integer>(1, FLAGS_ycsb_record_count + 1),
      [&](const tbb::blocked_range<Integer> &range) {
        ...
        for (auto key = range.begin(); key < range.end(); key++) {
          ...
          RandomGenerator::GetRandRepetitiveString(payload.data(), 100UL,
                                                   payload_sz);
          db->ui << "INSERT INTO YCSB_TABLE (my_key, my_payload) VALUES (?, ?)"
                 << key << payload;
        }
  ...
  threads.emplace_back(
      db->StartProfilingThread("sqlite", keep_running, completed_txn));
    ...
    threads.emplace_back([&]() {
      ...
      while (keep_running.load()) {
        ...
        if (RandomGenerator::GetRandU64(0, 100) <= FLAGS_ycsb_read_ratio) {
          payload.clear();
          db->ui << "SELECT * FROM YCSB_TABLE WHERE my_key = ?;"
                 << access_key >>
              [&]([[maybe_unused]] uint64_t my_key,
                  std::vector<uint8_t> my_payload) {
          ...
          RandomGenerator::GetRandRepetitiveString(payload.data(), 100UL,
                                                   payload_sz);
          db->ui << "UPDATE YCSB_TABLE SET my_payload = ? WHERE my_key = ?;"
                 << payload << access_key;
        }
