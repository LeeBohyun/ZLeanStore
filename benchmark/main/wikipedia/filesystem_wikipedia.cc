...
auto main(int argc, char **argv) -> int {
  ...
  auto wiki = std::make_unique<
      wiki::WikipediaReadOnly<FilesystemAdapter, wiki::FileRelation>>(false,
                                                                      db.get());
  ...
  tbb::parallel_for(
      tbb::blocked_range<UInteger>(1, wiki->RequiredNumberOfEntries() + 1),
      [&](const tbb::blocked_range<UInteger> &range) {
        wiki->LoadInitialDataWithoutBlobRep(range);
      });
  auto time_end = std::chrono::high_resolution_clock::now();
  LOG_INFO("Complete loading. Off-row records: %u - In-row records: %u",
           wiki->offrow_count.load(), wiki->inrow_count.load());
  ...
  if (FLAGS_wiki_clear_cache_before_expr) {
    db->DropCache();
  }
