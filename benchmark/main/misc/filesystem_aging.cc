...
auto main(int argc, char **argv) -> int {
  ...
  auto db = std::make_unique<FilesystemAsDB>(FLAGS_db_path,
                                             FLAGS_fs_enable_fsync, true);
  ...
  for (size_t w_id = 0; w_id < FLAGS_worker_count; w_id++) {
    threads.emplace_back([&, w_id]() {
      while (keep_running.load()) {
        auto is_alloc =
            RandomGenerator::GetRandU64(0, 100) <= FLAGS_alloc_ratio;
        if (is_alloc) {
          // Generate key and payload
          auto r_key = uid.fetch_add(1);
          auto payload_sz = ycsb::YCSBWorkloadInterface::PayloadSize();
          RandomGenerator::GetRandRepetitiveString(payloads[w_id].get(), 100UL,
                                                   payload_sz);
          relation.InsertRawPayload({r_key},
                                    {payloads[w_id].get(), payload_sz});
        } else {
