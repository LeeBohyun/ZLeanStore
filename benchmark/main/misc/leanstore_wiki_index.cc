...
/**
 * @brief Exec cmd: ./benchmark/LeanStore_IndexWiki -worker_count=1
 * -bm_virtual_gb=128 -bm_physical_gb=32 -db_path=/dev/nvme0n1p3
 * -blob_logging_variant={1,2}
 *
 * *IMPORTANT*: Please remove all short strings (which is a corner case in this
 * LeanStore's BLOB impl) FOr the evaluation, all strings whose size is < 100
 * bytes are removed
 */
auto main(int argc, char **argv) -> int {
  ...
  bh_index.SetComparisonOperator(leanstore::ComparisonOperator::BLOB_STATE);
  ...
  while (std::getline(articles_file, line)) {
    ...
    auto blob = std::span<uint8_t>(
        reinterpret_cast<uint8_t *>(const_cast<char *>(yyjson_get_str(text))),
        static_cast<uint64_t>(yyjson_get_len(text)));
    articles.emplace_back(yyjson_get_str(text),
                          static_cast<uint64_t>(yyjson_get_len(text)));
  ...
  wiki_relation.MiniTransactionWrapper([&]() {
    LOG_DEBUG("Record count: %lu - No articles: %lu", wiki_relation.Count(),
              articles.size());
  ...
  if (FLAGS_blob_indexing_variant == 1) {
    // Index construction
    LOG_INFO("Start building Blob State Index");
    time_start = std::chrono::high_resolution_clock::now();
    wiki_relation.MiniTransactionWrapper([&]() {
      wiki_relation.Scan(
          {0}, [&](const BlobRelation::Key &key, const BlobRelation &record) {
            auto bh =
                reinterpret_cast<const leanstore::BlobState *>(record.payload);
            ...
            if (FLAGS_wiki_index_evaluate_deduplication) {
              ...
              auto blob = std::span<uint8_t>(
                  reinterpret_cast<uint8_t *>(articles[id].data()),
                  articles[id].size());
              [[maybe_unused]] auto found =
                  bh_index.LookUpBlob(blob, [&](const auto &rec) {
                    if (key.my_key != rec.article_id) {
                      LOG_WARN("Duplication: Line %lu and %lu", key.my_key,
                               rec.article_id);
                    }
                  });
              assert(found);
    ...
    bh_index.MiniTransactionWrapper(
        [&]() { bh_index_size = bh_index.RelationSize(); });
    LOG_INFO(
        "Blob State Index build complete. Time: %lu ms - Space used: %.4f MB",
        std::chrono::duration_cast<std::chrono::milliseconds>(time_end -
                                                              time_start)
            .count(),
        bh_index_size);
  ...
  if (FLAGS_blob_indexing_variant == 2) {
    ...
    time_start = std::chrono::high_resolution_clock::now();
    wiki_relation.MiniTransactionWrapper([&]() {
      wiki_relation.Scan({0}, [&](const BlobRelation::Key &key,
                                  const BlobRelation &record) {
        auto blob_state =
            reinterpret_cast<const leanstore::BlobState *>(record.payload);
        db->LoadBlob(
            blob_state,
            [&](std::span<const uint8_t> blob) {
              ...
              prefix_key.prefix_length =
                  std::min(blob.size(), wiki::INDEX_PREFIX_LENGTH);
              std::memcpy(prefix_key.value, blob.data(),
                          prefix_key.prefix_length);
              if (FLAGS_wiki_index_evaluate_deduplication) {
                if (prefix_index.LookUp(
                        prefix_key, []([[maybe_unused]] const auto &rec) {})) {
                  rejection++;
                }
              }
    ...
    prefix_index.MiniTransactionWrapper(
        [&]() { prefix_index_size = prefix_index.RelationSize(); });
    LOG_INFO("Prefix Index build complete. Time: %lu ms - Space used: %.4f MB",
             ...
             prefix_index_size);
    if (FLAGS_wiki_index_evaluate_deduplication) {
      LOG_INFO("Prefix Index: Reject %u records", rejection);
    }
  }
  ...
  for (size_t w_id = 0; w_id < FLAGS_worker_count; w_id++) {
    db->worker_pool.ScheduleAsyncJob(w_id, [&]() {
      ...
      while (keep_running.load()) {
        ...
        auto blob =
            std::span<uint8_t>(reinterpret_cast<uint8_t *>(articles[id].data()),
                               articles[id].size());

        db->StartTransaction();
        switch (FLAGS_blob_indexing_variant) {
        case 1:
          found = bh_index.LookUpBlob(
              blob, [&](const auto &rec) { id = rec.article_id; });
          Ensure(found);
          break;
        case 2:
          found = false;
           ...
           *  call extra SHA-256 to validate if the found doc is what we we are
           * looking for 45% percentile are bigger than
           * wiki::INDEX_PREFIX_LENGTH Average size of object >
           * wiki::INDEX_PREFIX_LENGTH: 4393 bytes
           */
  ...
  LOG_INFO("Total completed txn %lu - Space used: %.4f GB - WAL size: %.4f GB "
           ...
           db->AllocatedSize(), db->WALSize(), not_found);
  e.stopCounters();
