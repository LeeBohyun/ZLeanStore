...
#include "benchmark/url/workload.h"
...
// using AdapterClass = url::URL<LeanStoreAdapter, url::Relation<1>>;
using AdapterClass =
    url::URL<LeanStoreAdapter, leanstore::schema::InrowBlobRelation<0>>;

// using AdapterClass = url::URL<LeanStoreAdapter>;
auto main(int argc, char **argv) -> int {
  gflags::SetUsageMessage("Leanstore URL");
  gflags::ParseCommandLineFlags(&argc, &argv, true);

  if (!FLAGS_url_random_payload) {
    FLAGS_url_max_payload_size = FLAGS_url_payload_size;
  }

  if (FLAGS_url_record_count > 6393703) {
    FLAGS_url_record_count = 6393700;
  }
  ...
  std::this_thread::sleep_for(std::chrono::seconds(FLAGS_url_exec_seconds));
  keep_running = false;
